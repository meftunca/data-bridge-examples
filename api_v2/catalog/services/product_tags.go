// ProductTagsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for ProductTags resources with:
...
// IProductTagsService defines the business logic interface for ProductTags operations.
//
// This interface abstracts all business operations for ProductTags resources, providing:
...
type IProductTagsService interface {
	UpdateProductTags(productId types.URID, tagId types.URID, data structures.ProductTagsEdit) error
	UpdateProductTagsMultiple(data []structures.ProductTagsBatchUpdate) error
	CreateProductTags(data structures.ProductTagsForm) (structures.ProductTagsForm, error)
	CreateProductTagsMultiple(data []structures.ProductTagsForm) ([]structures.ProductTagsForm, error)
	DeleteProductTags(productId types.URID, tagId types.URID) error
	DeleteProductTagsMultiple(identities []structures.ProductTagsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// ProductTagsService implements IProductTagsService with enterprise-grade business logic.
//
...
type ProductTagsService struct {
...
//	})
func (s *ProductTagsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *ProductTagsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *ProductTagsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyProductTagsCreateDefaults(data *structures.ProductTagsForm) {

}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *ProductTagsService) UpdateProductTags(productId types.URID, tagId types.URID, data structures.ProductTagsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.ProductTags{}).Where("product_id = ?", productId).Where("tag_id = ?", tagId).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateProductTags")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateProductTags: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "ProductTags",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateProductTags: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("catalog.product_tags", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *ProductTagsService) UpdateProductTagsMultiple(data []structures.ProductTagsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateProductTagsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateProductTagsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateProductTagsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateProductTagsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.ProductTags{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateProductTagsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateProductTagsMultiple", "not_found_error")
		}
...
func (s *ProductTagsService) CreateProductTags(data structures.ProductTagsForm) (structures.ProductTagsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyProductTagsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateProductTags")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateProductTags: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("catalog.product_tags", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *ProductTagsService) CreateProductTagsMultiple(data []structures.ProductTagsForm) ([]structures.ProductTagsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateProductTagsMultiple", "validation_error")
	}

	for i := range data {
		applyProductTagsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateProductTagsMultiple")
		}
		return nil
	})
...
func (s *ProductTagsService) DeleteProductTags(productId types.URID, tagId types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.ProductTags{}).Where("product_id = ?", productId).Where("tag_id = ?", tagId).Delete(&structures.ProductTags{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteProductTags")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteProductTags: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteProductTags: %w", err)
		}

		go sseRuntime.Publish("catalog.product_tags", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *ProductTagsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructProductTags(pathParams)
	if err != nil {
...
func buildWhereMapFromStructProductTags(value interface{}) (map[string]interface{}, error) {
...
func (s *ProductTagsService) DeleteProductTagsMultiple(identities []structures.ProductTagsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructProductTags(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.ProductTags{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteProductTagsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteProductTagsMultiple", "not_found_error")
		}
