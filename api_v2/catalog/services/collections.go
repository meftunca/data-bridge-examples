// CollectionsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for Collections resources with:
...
// ICollectionsService defines the business logic interface for Collections operations.
//
// This interface abstracts all business operations for Collections resources, providing:
...
type ICollectionsService interface {
	UpdateCollections(id types.URID, data structures.CollectionsEdit) error
	UpdateCollectionsMultiple(data []structures.CollectionsBatchUpdate) error
	CreateCollections(data structures.CollectionsForm) (structures.CollectionsForm, error)
	CreateCollectionsMultiple(data []structures.CollectionsForm) ([]structures.CollectionsForm, error)
	DeleteCollections(id types.URID) error
	DeleteCollectionsMultiple(identities []structures.CollectionsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// CollectionsService implements ICollectionsService with enterprise-grade business logic.
//
...
//	})
func (s *CollectionsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *CollectionsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *CollectionsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyCollectionsCreateDefaults(data *structures.CollectionsForm) {

}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *CollectionsService) UpdateCollections(id types.URID, data structures.CollectionsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Collections{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateCollections")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateCollections: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "Collections",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateCollections: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("catalog.collections", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *CollectionsService) UpdateCollectionsMultiple(data []structures.CollectionsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateCollectionsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateCollectionsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateCollectionsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateCollectionsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.Collections{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateCollectionsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateCollectionsMultiple", "not_found_error")
		}
...
func (s *CollectionsService) CreateCollections(data structures.CollectionsForm) (structures.CollectionsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyCollectionsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateCollections")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateCollections: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("catalog.collections", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *CollectionsService) CreateCollectionsMultiple(data []structures.CollectionsForm) ([]structures.CollectionsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateCollectionsMultiple", "validation_error")
	}

	for i := range data {
		applyCollectionsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateCollectionsMultiple")
		}
		return nil
	})
...
func (s *CollectionsService) DeleteCollections(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Collections{}).Where("id = ?", id).Delete(&structures.Collections{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteCollections")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteCollections: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteCollections: %w", err)
		}

		go sseRuntime.Publish("catalog.collections", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *CollectionsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructCollections(pathParams)
	if err != nil {
...
func buildWhereMapFromStructCollections(value interface{}) (map[string]interface{}, error) {
...
func (s *CollectionsService) DeleteCollectionsMultiple(identities []structures.CollectionsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructCollections(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.Collections{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteCollectionsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteCollectionsMultiple", "not_found_error")
		}
