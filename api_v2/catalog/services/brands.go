// BrandsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for Brands resources with:
...
// IBrandsService defines the business logic interface for Brands operations.
//
// This interface abstracts all business operations for Brands resources, providing:
...
type IBrandsService interface {
	UpdateBrands(id types.URID, data structures.BrandsEdit) error
	UpdateBrandsMultiple(data []structures.BrandsBatchUpdate) error
	CreateBrands(data structures.BrandsForm) (structures.BrandsForm, error)
	CreateBrandsMultiple(data []structures.BrandsForm) ([]structures.BrandsForm, error)
	DeleteBrands(id types.URID) error
	DeleteBrandsMultiple(identities []structures.BrandsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// BrandsService implements IBrandsService with enterprise-grade business logic.
//
...
//	})
func (s *BrandsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *BrandsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *BrandsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyBrandsCreateDefaults(data *structures.BrandsForm) {

}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *BrandsService) UpdateBrands(id types.URID, data structures.BrandsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Brands{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateBrands")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateBrands: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "Brands",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateBrands: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("catalog.brands", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *BrandsService) UpdateBrandsMultiple(data []structures.BrandsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateBrandsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateBrandsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateBrandsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateBrandsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.Brands{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateBrandsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateBrandsMultiple", "not_found_error")
		}
...
func (s *BrandsService) CreateBrands(data structures.BrandsForm) (structures.BrandsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyBrandsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateBrands")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateBrands: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("catalog.brands", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *BrandsService) CreateBrandsMultiple(data []structures.BrandsForm) ([]structures.BrandsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateBrandsMultiple", "validation_error")
	}

	for i := range data {
		applyBrandsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateBrandsMultiple")
		}
		return nil
	})
...
func (s *BrandsService) DeleteBrands(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Brands{}).Where("id = ?", id).Delete(&structures.Brands{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteBrands")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteBrands: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteBrands: %w", err)
		}

		go sseRuntime.Publish("catalog.brands", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *BrandsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructBrands(pathParams)
	if err != nil {
...
func buildWhereMapFromStructBrands(value interface{}) (map[string]interface{}, error) {
...
func (s *BrandsService) DeleteBrandsMultiple(identities []structures.BrandsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructBrands(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.Brands{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteBrandsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteBrandsMultiple", "not_found_error")
		}
