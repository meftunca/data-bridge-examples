// ProductVariantsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for ProductVariants resources with:
...
// IProductVariantsService defines the business logic interface for ProductVariants operations.
//
// This interface abstracts all business operations for ProductVariants resources, providing:
...
type IProductVariantsService interface {
	UpdateProductVariants(id types.URID, data structures.ProductVariantsEdit) error
	UpdateProductVariantsMultiple(data []structures.ProductVariantsBatchUpdate) error
	CreateProductVariants(data structures.ProductVariantsForm) (structures.ProductVariantsForm, error)
	CreateProductVariantsMultiple(data []structures.ProductVariantsForm) ([]structures.ProductVariantsForm, error)
	DeleteProductVariants(id types.URID) error
	DeleteProductVariantsMultiple(identities []structures.ProductVariantsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// ProductVariantsService implements IProductVariantsService with enterprise-grade business logic.
//
...
//	})
func (s *ProductVariantsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *ProductVariantsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *ProductVariantsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyProductVariantsCreateDefaults(data *structures.ProductVariantsForm) {

}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *ProductVariantsService) UpdateProductVariants(id types.URID, data structures.ProductVariantsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.ProductVariants{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateProductVariants")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateProductVariants: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "ProductVariants",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateProductVariants: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("catalog.product_variants", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *ProductVariantsService) UpdateProductVariantsMultiple(data []structures.ProductVariantsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateProductVariantsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateProductVariantsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateProductVariantsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateProductVariantsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.ProductVariants{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateProductVariantsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateProductVariantsMultiple", "not_found_error")
		}
...
func (s *ProductVariantsService) CreateProductVariants(data structures.ProductVariantsForm) (structures.ProductVariantsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyProductVariantsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateProductVariants")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateProductVariants: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("catalog.product_variants", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *ProductVariantsService) CreateProductVariantsMultiple(data []structures.ProductVariantsForm) ([]structures.ProductVariantsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateProductVariantsMultiple", "validation_error")
	}

	for i := range data {
		applyProductVariantsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateProductVariantsMultiple")
		}
		return nil
	})
...
func (s *ProductVariantsService) DeleteProductVariants(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.ProductVariants{}).Where("id = ?", id).Delete(&structures.ProductVariants{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteProductVariants")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteProductVariants: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteProductVariants: %w", err)
		}

		go sseRuntime.Publish("catalog.product_variants", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *ProductVariantsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructProductVariants(pathParams)
	if err != nil {
...
func buildWhereMapFromStructProductVariants(value interface{}) (map[string]interface{}, error) {
...
func (s *ProductVariantsService) DeleteProductVariantsMultiple(identities []structures.ProductVariantsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructProductVariants(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.ProductVariants{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteProductVariantsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteProductVariantsMultiple", "not_found_error")
		}
