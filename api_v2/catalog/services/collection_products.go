// CollectionProductsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for CollectionProducts resources with:
...
// ICollectionProductsService defines the business logic interface for CollectionProducts operations.
//
// This interface abstracts all business operations for CollectionProducts resources, providing:
...
type ICollectionProductsService interface {
	UpdateCollectionProducts(id types.URID, data structures.CollectionProductsEdit) error
	UpdateCollectionProductsMultiple(data []structures.CollectionProductsBatchUpdate) error
	CreateCollectionProducts(data structures.CollectionProductsForm) (structures.CollectionProductsForm, error)
	CreateCollectionProductsMultiple(data []structures.CollectionProductsForm) ([]structures.CollectionProductsForm, error)
	DeleteCollectionProducts(id types.URID) error
	DeleteCollectionProductsMultiple(identities []structures.CollectionProductsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// CollectionProductsService implements ICollectionProductsService with enterprise-grade business logic.
//
...
type CollectionProductsService struct {
...
//	})
func (s *CollectionProductsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *CollectionProductsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *CollectionProductsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyCollectionProductsCreateDefaults(data *structures.CollectionProductsForm) {

}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *CollectionProductsService) UpdateCollectionProducts(id types.URID, data structures.CollectionProductsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.CollectionProducts{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateCollectionProducts")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateCollectionProducts: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "CollectionProducts",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateCollectionProducts: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("catalog.collection_products", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *CollectionProductsService) UpdateCollectionProductsMultiple(data []structures.CollectionProductsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateCollectionProductsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateCollectionProductsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateCollectionProductsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateCollectionProductsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.CollectionProducts{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateCollectionProductsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateCollectionProductsMultiple", "not_found_error")
		}
...
func (s *CollectionProductsService) CreateCollectionProducts(data structures.CollectionProductsForm) (structures.CollectionProductsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyCollectionProductsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateCollectionProducts")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateCollectionProducts: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("catalog.collection_products", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *CollectionProductsService) CreateCollectionProductsMultiple(data []structures.CollectionProductsForm) ([]structures.CollectionProductsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateCollectionProductsMultiple", "validation_error")
	}

	for i := range data {
		applyCollectionProductsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateCollectionProductsMultiple")
		}
		return nil
	})
...
func (s *CollectionProductsService) DeleteCollectionProducts(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.CollectionProducts{}).Where("id = ?", id).Delete(&structures.CollectionProducts{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteCollectionProducts")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteCollectionProducts: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteCollectionProducts: %w", err)
		}

		go sseRuntime.Publish("catalog.collection_products", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *CollectionProductsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructCollectionProducts(pathParams)
	if err != nil {
...
func buildWhereMapFromStructCollectionProducts(value interface{}) (map[string]interface{}, error) {
...
func (s *CollectionProductsService) DeleteCollectionProductsMultiple(identities []structures.CollectionProductsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructCollectionProducts(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.CollectionProducts{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteCollectionProductsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteCollectionProductsMultiple", "not_found_error")
		}
