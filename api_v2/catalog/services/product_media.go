// ProductMediaService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for ProductMedia resources with:
...
// IProductMediaService defines the business logic interface for ProductMedia operations.
//
// This interface abstracts all business operations for ProductMedia resources, providing:
...
type IProductMediaService interface {
	UpdateProductMedia(id types.URID, data structures.ProductMediaEdit) error
	UpdateProductMediaMultiple(data []structures.ProductMediaBatchUpdate) error
	CreateProductMedia(data structures.ProductMediaForm) (structures.ProductMediaForm, error)
	CreateProductMediaMultiple(data []structures.ProductMediaForm) ([]structures.ProductMediaForm, error)
	DeleteProductMedia(id types.URID) error
	DeleteProductMediaMultiple(identities []structures.ProductMediaIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// ProductMediaService implements IProductMediaService with enterprise-grade business logic.
//
...
//	})
func (s *ProductMediaService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *ProductMediaService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *ProductMediaService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyProductMediaCreateDefaults(data *structures.ProductMediaForm) {

}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *ProductMediaService) UpdateProductMedia(id types.URID, data structures.ProductMediaEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.ProductMedia{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateProductMedia")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateProductMedia: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateProductMedia: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("catalog.product_media", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *ProductMediaService) UpdateProductMediaMultiple(data []structures.ProductMediaBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateProductMediaMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateProductMediaMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateProductMediaMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateProductMediaMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.ProductMedia{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateProductMediaMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateProductMediaMultiple", "not_found_error")
		}
...
func (s *ProductMediaService) CreateProductMedia(data structures.ProductMediaForm) (structures.ProductMediaForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyProductMediaCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateProductMedia")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateProductMedia: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("catalog.product_media", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *ProductMediaService) CreateProductMediaMultiple(data []structures.ProductMediaForm) ([]structures.ProductMediaForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateProductMediaMultiple", "validation_error")
	}

	for i := range data {
		applyProductMediaCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateProductMediaMultiple")
		}
		return nil
	})
...
func (s *ProductMediaService) DeleteProductMedia(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.ProductMedia{}).Where("id = ?", id).Delete(&structures.ProductMedia{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteProductMedia")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteProductMedia: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteProductMedia: %w", err)
		}

		go sseRuntime.Publish("catalog.product_media", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *ProductMediaService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructProductMedia(pathParams)
	if err != nil {
...
func buildWhereMapFromStructProductMedia(value interface{}) (map[string]interface{}, error) {
...
func (s *ProductMediaService) DeleteProductMediaMultiple(identities []structures.ProductMediaIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructProductMedia(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.ProductMedia{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteProductMediaMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteProductMediaMultiple", "not_found_error")
		}
