// ProductReviewsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for ProductReviews resources with:
...
// IProductReviewsService defines the business logic interface for ProductReviews operations.
//
// This interface abstracts all business operations for ProductReviews resources, providing:
...
type IProductReviewsService interface {
	UpdateProductReviews(id types.URID, data structures.ProductReviewsEdit) error
	UpdateProductReviewsMultiple(data []structures.ProductReviewsBatchUpdate) error
	CreateProductReviews(data structures.ProductReviewsForm) (structures.ProductReviewsForm, error)
	CreateProductReviewsMultiple(data []structures.ProductReviewsForm) ([]structures.ProductReviewsForm, error)
	DeleteProductReviews(id types.URID) error
	DeleteProductReviewsMultiple(identities []structures.ProductReviewsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// ProductReviewsService implements IProductReviewsService with enterprise-grade business logic.
//
...
//	})
func (s *ProductReviewsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *ProductReviewsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *ProductReviewsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyProductReviewsCreateDefaults(data *structures.ProductReviewsForm) {

}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *ProductReviewsService) UpdateProductReviews(id types.URID, data structures.ProductReviewsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.ProductReviews{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateProductReviews")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateProductReviews: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "ProductReviews",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateProductReviews: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("catalog.product_reviews", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *ProductReviewsService) UpdateProductReviewsMultiple(data []structures.ProductReviewsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateProductReviewsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateProductReviewsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateProductReviewsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateProductReviewsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.ProductReviews{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateProductReviewsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateProductReviewsMultiple", "not_found_error")
		}
...
func (s *ProductReviewsService) CreateProductReviews(data structures.ProductReviewsForm) (structures.ProductReviewsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyProductReviewsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateProductReviews")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateProductReviews: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("catalog.product_reviews", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *ProductReviewsService) CreateProductReviewsMultiple(data []structures.ProductReviewsForm) ([]structures.ProductReviewsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateProductReviewsMultiple", "validation_error")
	}

	for i := range data {
		applyProductReviewsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateProductReviewsMultiple")
		}
		return nil
	})
...
func (s *ProductReviewsService) DeleteProductReviews(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.ProductReviews{}).Where("id = ?", id).Delete(&structures.ProductReviews{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteProductReviews")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteProductReviews: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteProductReviews: %w", err)
		}

		go sseRuntime.Publish("catalog.product_reviews", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *ProductReviewsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructProductReviews(pathParams)
	if err != nil {
...
func buildWhereMapFromStructProductReviews(value interface{}) (map[string]interface{}, error) {
...
func (s *ProductReviewsService) DeleteProductReviewsMultiple(identities []structures.ProductReviewsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructProductReviews(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.ProductReviews{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteProductReviewsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteProductReviewsMultiple", "not_found_error")
		}
