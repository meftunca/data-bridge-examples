// PriceHistoryService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for PriceHistory resources with:
...
// IPriceHistoryService defines the business logic interface for PriceHistory operations.
//
// This interface abstracts all business operations for PriceHistory resources, providing:
...
type IPriceHistoryService interface {
	UpdatePriceHistory(id types.URID, data structures.PriceHistoryEdit) error
	UpdatePriceHistoryMultiple(data []structures.PriceHistoryBatchUpdate) error
	CreatePriceHistory(data structures.PriceHistoryForm) (structures.PriceHistoryForm, error)
	CreatePriceHistoryMultiple(data []structures.PriceHistoryForm) ([]structures.PriceHistoryForm, error)
	DeletePriceHistory(id types.URID) error
	DeletePriceHistoryMultiple(identities []structures.PriceHistoryIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// PriceHistoryService implements IPriceHistoryService with enterprise-grade business logic.
//
...
//	})
func (s *PriceHistoryService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *PriceHistoryService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *PriceHistoryService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyPriceHistoryCreateDefaults(data *structures.PriceHistoryForm) {

}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *PriceHistoryService) UpdatePriceHistory(id types.URID, data structures.PriceHistoryEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.PriceHistory{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdatePriceHistory")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdatePriceHistory: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "PriceHistory",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdatePriceHistory: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("catalog.price_history", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *PriceHistoryService) UpdatePriceHistoryMultiple(data []structures.PriceHistoryBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdatePriceHistoryMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdatePriceHistoryMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdatePriceHistoryMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdatePriceHistoryMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.PriceHistory{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdatePriceHistoryMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdatePriceHistoryMultiple", "not_found_error")
		}
...
func (s *PriceHistoryService) CreatePriceHistory(data structures.PriceHistoryForm) (structures.PriceHistoryForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyPriceHistoryCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreatePriceHistory")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreatePriceHistory: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("catalog.price_history", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *PriceHistoryService) CreatePriceHistoryMultiple(data []structures.PriceHistoryForm) ([]structures.PriceHistoryForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreatePriceHistoryMultiple", "validation_error")
	}

	for i := range data {
		applyPriceHistoryCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreatePriceHistoryMultiple")
		}
		return nil
	})
...
func (s *PriceHistoryService) DeletePriceHistory(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.PriceHistory{}).Where("id = ?", id).Delete(&structures.PriceHistory{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeletePriceHistory")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeletePriceHistory: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeletePriceHistory: %w", err)
		}

		go sseRuntime.Publish("catalog.price_history", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *PriceHistoryService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructPriceHistory(pathParams)
	if err != nil {
...
func buildWhereMapFromStructPriceHistory(value interface{}) (map[string]interface{}, error) {
...
func (s *PriceHistoryService) DeletePriceHistoryMultiple(identities []structures.PriceHistoryIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructPriceHistory(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.PriceHistory{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeletePriceHistoryMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeletePriceHistoryMultiple", "not_found_error")
		}
