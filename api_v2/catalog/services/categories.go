// CategoriesService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for Categories resources with:
...
// ICategoriesService defines the business logic interface for Categories operations.
//
// This interface abstracts all business operations for Categories resources, providing:
...
type ICategoriesService interface {
	UpdateCategories(id types.URID, data structures.CategoriesEdit) error
	UpdateCategoriesMultiple(data []structures.CategoriesBatchUpdate) error
	CreateCategories(data structures.CategoriesForm) (structures.CategoriesForm, error)
	CreateCategoriesMultiple(data []structures.CategoriesForm) ([]structures.CategoriesForm, error)
	DeleteCategories(id types.URID) error
	DeleteCategoriesMultiple(identities []structures.CategoriesIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// CategoriesService implements ICategoriesService with enterprise-grade business logic.
//
...
//	})
func (s *CategoriesService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *CategoriesService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *CategoriesService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyCategoriesCreateDefaults(data *structures.CategoriesForm) {

}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *CategoriesService) UpdateCategories(id types.URID, data structures.CategoriesEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Categories{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateCategories")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateCategories: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateCategories: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("catalog.categories", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *CategoriesService) UpdateCategoriesMultiple(data []structures.CategoriesBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateCategoriesMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateCategoriesMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateCategoriesMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateCategoriesMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.Categories{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateCategoriesMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateCategoriesMultiple", "not_found_error")
		}
...
func (s *CategoriesService) CreateCategories(data structures.CategoriesForm) (structures.CategoriesForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyCategoriesCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateCategories")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateCategories: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("catalog.categories", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *CategoriesService) CreateCategoriesMultiple(data []structures.CategoriesForm) ([]structures.CategoriesForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateCategoriesMultiple", "validation_error")
	}

	for i := range data {
		applyCategoriesCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateCategoriesMultiple")
		}
		return nil
	})
...
func (s *CategoriesService) DeleteCategories(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Categories{}).Where("id = ?", id).Delete(&structures.Categories{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteCategories")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteCategories: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteCategories: %w", err)
		}

		go sseRuntime.Publish("catalog.categories", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *CategoriesService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructCategories(pathParams)
	if err != nil {
...
func buildWhereMapFromStructCategories(value interface{}) (map[string]interface{}, error) {
...
func (s *CategoriesService) DeleteCategoriesMultiple(identities []structures.CategoriesIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructCategories(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.Categories{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteCategoriesMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteCategoriesMultiple", "not_found_error")
		}
