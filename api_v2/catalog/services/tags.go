// TagsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for Tags resources with:
...
// ITagsService defines the business logic interface for Tags operations.
//
// This interface abstracts all business operations for Tags resources, providing:
...
type ITagsService interface {
	UpdateTags(id types.URID, data structures.TagsEdit) error
	UpdateTagsMultiple(data []structures.TagsBatchUpdate) error
	CreateTags(data structures.TagsForm) (structures.TagsForm, error)
	CreateTagsMultiple(data []structures.TagsForm) ([]structures.TagsForm, error)
	DeleteTags(id types.URID) error
	DeleteTagsMultiple(identities []structures.TagsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// TagsService implements ITagsService with enterprise-grade business logic.
//
...
//	})
func (s *TagsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *TagsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *TagsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyTagsCreateDefaults(data *structures.TagsForm) {

}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *TagsService) UpdateTags(id types.URID, data structures.TagsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Tags{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateTags")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateTags: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateTags: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("catalog.tags", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *TagsService) UpdateTagsMultiple(data []structures.TagsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateTagsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateTagsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateTagsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateTagsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.Tags{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateTagsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateTagsMultiple", "not_found_error")
		}
...
func (s *TagsService) CreateTags(data structures.TagsForm) (structures.TagsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyTagsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateTags")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateTags: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("catalog.tags", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *TagsService) CreateTagsMultiple(data []structures.TagsForm) ([]structures.TagsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateTagsMultiple", "validation_error")
	}

	for i := range data {
		applyTagsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateTagsMultiple")
		}
		return nil
	})
...
func (s *TagsService) DeleteTags(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Tags{}).Where("id = ?", id).Delete(&structures.Tags{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteTags")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteTags: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteTags: %w", err)
		}

		go sseRuntime.Publish("catalog.tags", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *TagsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructTags(pathParams)
	if err != nil {
...
func buildWhereMapFromStructTags(value interface{}) (map[string]interface{}, error) {
...
func (s *TagsService) DeleteTagsMultiple(identities []structures.TagsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructTags(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.Tags{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteTagsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteTagsMultiple", "not_found_error")
		}
