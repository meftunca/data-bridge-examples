// ProductsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for Products resources with:
...
// IProductsService defines the business logic interface for Products operations.
//
// This interface abstracts all business operations for Products resources, providing:
...
type IProductsService interface {
	UpdateProducts(id types.URID, data structures.ProductsEdit) error
	UpdateProductsMultiple(data []structures.ProductsBatchUpdate) error
	CreateProducts(data structures.ProductsForm) (structures.ProductsForm, error)
	CreateProductsMultiple(data []structures.ProductsForm) ([]structures.ProductsForm, error)
	DeleteProducts(id types.URID) error
	DeleteProductsMultiple(identities []structures.ProductsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// ProductsService implements IProductsService with enterprise-grade business logic.
//
...
//	})
func (s *ProductsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *ProductsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *ProductsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyProductsCreateDefaults(data *structures.ProductsForm) {
	if data.Currency == "" {
		data.Currency = "USD"
	}
}
...
func (s *ProductsService) UpdateProducts(id types.URID, data structures.ProductsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Products{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateProducts")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateProducts: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateProducts: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("catalog.products", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *ProductsService) UpdateProductsMultiple(data []structures.ProductsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateProductsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateProductsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateProductsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateProductsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.Products{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateProductsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateProductsMultiple", "not_found_error")
		}
...
func (s *ProductsService) CreateProducts(data structures.ProductsForm) (structures.ProductsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyProductsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateProducts")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateProducts: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("catalog.products", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *ProductsService) CreateProductsMultiple(data []structures.ProductsForm) ([]structures.ProductsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateProductsMultiple", "validation_error")
	}

	for i := range data {
		applyProductsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateProductsMultiple")
		}
		return nil
	})
...
func (s *ProductsService) DeleteProducts(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Products{}).Where("id = ?", id).Delete(&structures.Products{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteProducts")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteProducts: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteProducts: %w", err)
		}

		go sseRuntime.Publish("catalog.products", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *ProductsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructProducts(pathParams)
	if err != nil {
...
func buildWhereMapFromStructProducts(value interface{}) (map[string]interface{}, error) {
...
func (s *ProductsService) DeleteProductsMultiple(identities []structures.ProductsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructProducts(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.Products{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteProductsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteProductsMultiple", "not_found_error")
		}
