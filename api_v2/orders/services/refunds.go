// RefundsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for Refunds resources with:
...
// IRefundsService defines the business logic interface for Refunds operations.
//
// This interface abstracts all business operations for Refunds resources, providing:
...
type IRefundsService interface {
	UpdateRefunds(id types.URID, data structures.RefundsEdit) error
	UpdateRefundsMultiple(data []structures.RefundsBatchUpdate) error
	CreateRefunds(data structures.RefundsForm) (structures.RefundsForm, error)
	CreateRefundsMultiple(data []structures.RefundsForm) ([]structures.RefundsForm, error)
	DeleteRefunds(id types.URID) error
	DeleteRefundsMultiple(identities []structures.RefundsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// RefundsService implements IRefundsService with enterprise-grade business logic.
//
...
//	})
func (s *RefundsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *RefundsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *RefundsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyRefundsCreateDefaults(data *structures.RefundsForm) {
	if data.Status == "" {
		data.Status = "pending"
	}
}
...
func (s *RefundsService) UpdateRefunds(id types.URID, data structures.RefundsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Refunds{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateRefunds")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateRefunds: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateRefunds: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("orders.refunds", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *RefundsService) UpdateRefundsMultiple(data []structures.RefundsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateRefundsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateRefundsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateRefundsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateRefundsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.Refunds{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateRefundsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateRefundsMultiple", "not_found_error")
		}
...
func (s *RefundsService) CreateRefunds(data structures.RefundsForm) (structures.RefundsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyRefundsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateRefunds")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateRefunds: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("orders.refunds", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *RefundsService) CreateRefundsMultiple(data []structures.RefundsForm) ([]structures.RefundsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateRefundsMultiple", "validation_error")
	}

	for i := range data {
		applyRefundsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateRefundsMultiple")
		}
		return nil
	})
...
func (s *RefundsService) DeleteRefunds(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Refunds{}).Where("id = ?", id).Delete(&structures.Refunds{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteRefunds")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteRefunds: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteRefunds: %w", err)
		}

		go sseRuntime.Publish("orders.refunds", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *RefundsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructRefunds(pathParams)
	if err != nil {
...
func buildWhereMapFromStructRefunds(value interface{}) (map[string]interface{}, error) {
...
func (s *RefundsService) DeleteRefundsMultiple(identities []structures.RefundsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructRefunds(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.Refunds{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteRefundsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteRefundsMultiple", "not_found_error")
		}
