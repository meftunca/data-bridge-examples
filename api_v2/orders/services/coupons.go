// CouponsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for Coupons resources with:
...
// ICouponsService defines the business logic interface for Coupons operations.
//
// This interface abstracts all business operations for Coupons resources, providing:
...
type ICouponsService interface {
	UpdateCoupons(id types.URID, data structures.CouponsEdit) error
	UpdateCouponsMultiple(data []structures.CouponsBatchUpdate) error
	CreateCoupons(data structures.CouponsForm) (structures.CouponsForm, error)
	CreateCouponsMultiple(data []structures.CouponsForm) ([]structures.CouponsForm, error)
	DeleteCoupons(id types.URID) error
	DeleteCouponsMultiple(identities []structures.CouponsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// CouponsService implements ICouponsService with enterprise-grade business logic.
//
...
//	})
func (s *CouponsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *CouponsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *CouponsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyCouponsCreateDefaults(data *structures.CouponsForm) {
	if data.DiscountType == "" {
		data.DiscountType = "percentage"
	}
	if !data.ValidFrom.Valid {
		data.ValidFrom = types.NullTime{Time: time.Now(), Valid: true}
	}
}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *CouponsService) UpdateCoupons(id types.URID, data structures.CouponsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Coupons{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateCoupons")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateCoupons: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "Coupons",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateCoupons: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("orders.coupons", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *CouponsService) UpdateCouponsMultiple(data []structures.CouponsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateCouponsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateCouponsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateCouponsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateCouponsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.Coupons{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateCouponsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateCouponsMultiple", "not_found_error")
		}
...
func (s *CouponsService) CreateCoupons(data structures.CouponsForm) (structures.CouponsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyCouponsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateCoupons")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateCoupons: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("orders.coupons", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *CouponsService) CreateCouponsMultiple(data []structures.CouponsForm) ([]structures.CouponsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateCouponsMultiple", "validation_error")
	}

	for i := range data {
		applyCouponsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateCouponsMultiple")
		}
		return nil
	})
...
func (s *CouponsService) DeleteCoupons(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Coupons{}).Where("id = ?", id).Delete(&structures.Coupons{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteCoupons")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteCoupons: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteCoupons: %w", err)
		}

		go sseRuntime.Publish("orders.coupons", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *CouponsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructCoupons(pathParams)
	if err != nil {
...
func buildWhereMapFromStructCoupons(value interface{}) (map[string]interface{}, error) {
...
func (s *CouponsService) DeleteCouponsMultiple(identities []structures.CouponsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructCoupons(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.Coupons{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteCouponsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteCouponsMultiple", "not_found_error")
		}
