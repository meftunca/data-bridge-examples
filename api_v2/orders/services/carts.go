// CartsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for Carts resources with:
...
// ICartsService defines the business logic interface for Carts operations.
//
// This interface abstracts all business operations for Carts resources, providing:
...
type ICartsService interface {
	UpdateCarts(id types.URID, data structures.CartsEdit) error
	UpdateCartsMultiple(data []structures.CartsBatchUpdate) error
	CreateCarts(data structures.CartsForm) (structures.CartsForm, error)
	CreateCartsMultiple(data []structures.CartsForm) ([]structures.CartsForm, error)
	DeleteCarts(id types.URID) error
	DeleteCartsMultiple(identities []structures.CartsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// CartsService implements ICartsService with enterprise-grade business logic.
//
...
//	})
func (s *CartsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *CartsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *CartsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyCartsCreateDefaults(data *structures.CartsForm) {

}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *CartsService) UpdateCarts(id types.URID, data structures.CartsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Carts{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateCarts")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateCarts: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateCarts: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("orders.carts", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *CartsService) UpdateCartsMultiple(data []structures.CartsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateCartsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateCartsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateCartsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateCartsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.Carts{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateCartsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateCartsMultiple", "not_found_error")
		}
...
func (s *CartsService) CreateCarts(data structures.CartsForm) (structures.CartsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyCartsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateCarts")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateCarts: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("orders.carts", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *CartsService) CreateCartsMultiple(data []structures.CartsForm) ([]structures.CartsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateCartsMultiple", "validation_error")
	}

	for i := range data {
		applyCartsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateCartsMultiple")
		}
		return nil
	})
...
func (s *CartsService) DeleteCarts(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Carts{}).Where("id = ?", id).Delete(&structures.Carts{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteCarts")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteCarts: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteCarts: %w", err)
		}

		go sseRuntime.Publish("orders.carts", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *CartsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructCarts(pathParams)
	if err != nil {
...
func buildWhereMapFromStructCarts(value interface{}) (map[string]interface{}, error) {
...
func (s *CartsService) DeleteCartsMultiple(identities []structures.CartsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructCarts(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.Carts{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteCartsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteCartsMultiple", "not_found_error")
		}
