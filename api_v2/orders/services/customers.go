// CustomersService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for Customers resources with:
...
// ICustomersService defines the business logic interface for Customers operations.
//
// This interface abstracts all business operations for Customers resources, providing:
...
type ICustomersService interface {
	UpdateCustomers(id types.URID, data structures.CustomersEdit) error
	UpdateCustomersMultiple(data []structures.CustomersBatchUpdate) error
	CreateCustomers(data structures.CustomersForm) (structures.CustomersForm, error)
	CreateCustomersMultiple(data []structures.CustomersForm) ([]structures.CustomersForm, error)
	DeleteCustomers(id types.URID) error
	DeleteCustomersMultiple(identities []structures.CustomersIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// CustomersService implements ICustomersService with enterprise-grade business logic.
//
...
//	})
func (s *CustomersService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *CustomersService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *CustomersService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyCustomersCreateDefaults(data *structures.CustomersForm) {
	if data.Tier == "" {
		data.Tier = "standard"
	}
}
...
func (s *CustomersService) UpdateCustomers(id types.URID, data structures.CustomersEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Customers{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateCustomers")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateCustomers: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateCustomers: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("orders.customers", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *CustomersService) UpdateCustomersMultiple(data []structures.CustomersBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateCustomersMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateCustomersMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateCustomersMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateCustomersMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.Customers{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateCustomersMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateCustomersMultiple", "not_found_error")
		}
...
func (s *CustomersService) CreateCustomers(data structures.CustomersForm) (structures.CustomersForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyCustomersCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateCustomers")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateCustomers: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("orders.customers", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *CustomersService) CreateCustomersMultiple(data []structures.CustomersForm) ([]structures.CustomersForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateCustomersMultiple", "validation_error")
	}

	for i := range data {
		applyCustomersCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateCustomersMultiple")
		}
		return nil
	})
...
func (s *CustomersService) DeleteCustomers(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Customers{}).Where("id = ?", id).Delete(&structures.Customers{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteCustomers")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteCustomers: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteCustomers: %w", err)
		}

		go sseRuntime.Publish("orders.customers", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *CustomersService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructCustomers(pathParams)
	if err != nil {
...
func buildWhereMapFromStructCustomers(value interface{}) (map[string]interface{}, error) {
...
func (s *CustomersService) DeleteCustomersMultiple(identities []structures.CustomersIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructCustomers(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.Customers{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteCustomersMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteCustomersMultiple", "not_found_error")
		}
