// OrderStatusHistoryService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for OrderStatusHistory resources with:
...
// IOrderStatusHistoryService defines the business logic interface for OrderStatusHistory operations.
//
// This interface abstracts all business operations for OrderStatusHistory resources, providing:
...
type IOrderStatusHistoryService interface {
	UpdateOrderStatusHistory(id types.URID, data structures.OrderStatusHistoryEdit) error
	UpdateOrderStatusHistoryMultiple(data []structures.OrderStatusHistoryBatchUpdate) error
	CreateOrderStatusHistory(data structures.OrderStatusHistoryForm) (structures.OrderStatusHistoryForm, error)
	CreateOrderStatusHistoryMultiple(data []structures.OrderStatusHistoryForm) ([]structures.OrderStatusHistoryForm, error)
	DeleteOrderStatusHistory(id types.URID) error
	DeleteOrderStatusHistoryMultiple(identities []structures.OrderStatusHistoryIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// OrderStatusHistoryService implements IOrderStatusHistoryService with enterprise-grade business logic.
//
...
//	})
func (s *OrderStatusHistoryService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *OrderStatusHistoryService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *OrderStatusHistoryService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyOrderStatusHistoryCreateDefaults(data *structures.OrderStatusHistoryForm) {

}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *OrderStatusHistoryService) UpdateOrderStatusHistory(id types.URID, data structures.OrderStatusHistoryEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.OrderStatusHistory{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateOrderStatusHistory")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateOrderStatusHistory: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "OrderStatusHistory",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateOrderStatusHistory: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("orders.order_status_history", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *OrderStatusHistoryService) UpdateOrderStatusHistoryMultiple(data []structures.OrderStatusHistoryBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateOrderStatusHistoryMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateOrderStatusHistoryMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateOrderStatusHistoryMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateOrderStatusHistoryMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.OrderStatusHistory{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateOrderStatusHistoryMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateOrderStatusHistoryMultiple", "not_found_error")
		}
...
func (s *OrderStatusHistoryService) CreateOrderStatusHistory(data structures.OrderStatusHistoryForm) (structures.OrderStatusHistoryForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyOrderStatusHistoryCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateOrderStatusHistory")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateOrderStatusHistory: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("orders.order_status_history", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *OrderStatusHistoryService) CreateOrderStatusHistoryMultiple(data []structures.OrderStatusHistoryForm) ([]structures.OrderStatusHistoryForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateOrderStatusHistoryMultiple", "validation_error")
	}

	for i := range data {
		applyOrderStatusHistoryCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateOrderStatusHistoryMultiple")
		}
		return nil
	})
...
func (s *OrderStatusHistoryService) DeleteOrderStatusHistory(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.OrderStatusHistory{}).Where("id = ?", id).Delete(&structures.OrderStatusHistory{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteOrderStatusHistory")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteOrderStatusHistory: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteOrderStatusHistory: %w", err)
		}

		go sseRuntime.Publish("orders.order_status_history", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *OrderStatusHistoryService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructOrderStatusHistory(pathParams)
	if err != nil {
...
func buildWhereMapFromStructOrderStatusHistory(value interface{}) (map[string]interface{}, error) {
...
func (s *OrderStatusHistoryService) DeleteOrderStatusHistoryMultiple(identities []structures.OrderStatusHistoryIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructOrderStatusHistory(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.OrderStatusHistory{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteOrderStatusHistoryMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteOrderStatusHistoryMultiple", "not_found_error")
		}
