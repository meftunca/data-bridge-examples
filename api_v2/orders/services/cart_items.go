// CartItemsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for CartItems resources with:
...
// ICartItemsService defines the business logic interface for CartItems operations.
//
// This interface abstracts all business operations for CartItems resources, providing:
...
type ICartItemsService interface {
	UpdateCartItems(id types.URID, data structures.CartItemsEdit) error
	UpdateCartItemsMultiple(data []structures.CartItemsBatchUpdate) error
	CreateCartItems(data structures.CartItemsForm) (structures.CartItemsForm, error)
	CreateCartItemsMultiple(data []structures.CartItemsForm) ([]structures.CartItemsForm, error)
	DeleteCartItems(id types.URID) error
	DeleteCartItemsMultiple(identities []structures.CartItemsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// CartItemsService implements ICartItemsService with enterprise-grade business logic.
//
...
//	})
func (s *CartItemsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *CartItemsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *CartItemsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyCartItemsCreateDefaults(data *structures.CartItemsForm) {
	if data.Quantity == 0 {
		data.Quantity = 1
	}
}
...
func (s *CartItemsService) UpdateCartItems(id types.URID, data structures.CartItemsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.CartItems{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateCartItems")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateCartItems: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "CartItems",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateCartItems: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("orders.cart_items", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *CartItemsService) UpdateCartItemsMultiple(data []structures.CartItemsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateCartItemsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateCartItemsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateCartItemsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateCartItemsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.CartItems{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateCartItemsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateCartItemsMultiple", "not_found_error")
		}
...
func (s *CartItemsService) CreateCartItems(data structures.CartItemsForm) (structures.CartItemsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyCartItemsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateCartItems")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateCartItems: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("orders.cart_items", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *CartItemsService) CreateCartItemsMultiple(data []structures.CartItemsForm) ([]structures.CartItemsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateCartItemsMultiple", "validation_error")
	}

	for i := range data {
		applyCartItemsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateCartItemsMultiple")
		}
		return nil
	})
...
func (s *CartItemsService) DeleteCartItems(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.CartItems{}).Where("id = ?", id).Delete(&structures.CartItems{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteCartItems")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteCartItems: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteCartItems: %w", err)
		}

		go sseRuntime.Publish("orders.cart_items", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *CartItemsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructCartItems(pathParams)
	if err != nil {
...
func buildWhereMapFromStructCartItems(value interface{}) (map[string]interface{}, error) {
...
func (s *CartItemsService) DeleteCartItemsMultiple(identities []structures.CartItemsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructCartItems(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.CartItems{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteCartItemsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteCartItemsMultiple", "not_found_error")
		}
