// PaymentsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for Payments resources with:
...
// IPaymentsService defines the business logic interface for Payments operations.
//
// This interface abstracts all business operations for Payments resources, providing:
...
type IPaymentsService interface {
	UpdatePayments(id types.URID, data structures.PaymentsEdit) error
	UpdatePaymentsMultiple(data []structures.PaymentsBatchUpdate) error
	CreatePayments(data structures.PaymentsForm) (structures.PaymentsForm, error)
	CreatePaymentsMultiple(data []structures.PaymentsForm) ([]structures.PaymentsForm, error)
	DeletePayments(id types.URID) error
	DeletePaymentsMultiple(identities []structures.PaymentsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// PaymentsService implements IPaymentsService with enterprise-grade business logic.
//
...
//	})
func (s *PaymentsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *PaymentsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *PaymentsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyPaymentsCreateDefaults(data *structures.PaymentsForm) {
	if data.Currency == "" {
		data.Currency = "USD"
	}
}
...
func (s *PaymentsService) UpdatePayments(id types.URID, data structures.PaymentsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Payments{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdatePayments")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdatePayments: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdatePayments: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("orders.payments", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *PaymentsService) UpdatePaymentsMultiple(data []structures.PaymentsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdatePaymentsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdatePaymentsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdatePaymentsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdatePaymentsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.Payments{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdatePaymentsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdatePaymentsMultiple", "not_found_error")
		}
...
func (s *PaymentsService) CreatePayments(data structures.PaymentsForm) (structures.PaymentsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyPaymentsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreatePayments")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreatePayments: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("orders.payments", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *PaymentsService) CreatePaymentsMultiple(data []structures.PaymentsForm) ([]structures.PaymentsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreatePaymentsMultiple", "validation_error")
	}

	for i := range data {
		applyPaymentsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreatePaymentsMultiple")
		}
		return nil
	})
...
func (s *PaymentsService) DeletePayments(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Payments{}).Where("id = ?", id).Delete(&structures.Payments{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeletePayments")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeletePayments: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeletePayments: %w", err)
		}

		go sseRuntime.Publish("orders.payments", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *PaymentsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructPayments(pathParams)
	if err != nil {
...
func buildWhereMapFromStructPayments(value interface{}) (map[string]interface{}, error) {
...
func (s *PaymentsService) DeletePaymentsMultiple(identities []structures.PaymentsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructPayments(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.Payments{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeletePaymentsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeletePaymentsMultiple", "not_found_error")
		}
