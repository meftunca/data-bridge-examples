// OrderItemsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for OrderItems resources with:
...
// IOrderItemsService defines the business logic interface for OrderItems operations.
//
// This interface abstracts all business operations for OrderItems resources, providing:
...
type IOrderItemsService interface {
	UpdateOrderItems(id types.URID, data structures.OrderItemsEdit) error
	UpdateOrderItemsMultiple(data []structures.OrderItemsBatchUpdate) error
	CreateOrderItems(data structures.OrderItemsForm) (structures.OrderItemsForm, error)
	CreateOrderItemsMultiple(data []structures.OrderItemsForm) ([]structures.OrderItemsForm, error)
	DeleteOrderItems(id types.URID) error
	DeleteOrderItemsMultiple(identities []structures.OrderItemsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// OrderItemsService implements IOrderItemsService with enterprise-grade business logic.
//
...
//	})
func (s *OrderItemsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *OrderItemsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *OrderItemsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyOrderItemsCreateDefaults(data *structures.OrderItemsForm) {
	if data.Quantity == 0 {
		data.Quantity = 1
	}
}
...
func (s *OrderItemsService) UpdateOrderItems(id types.URID, data structures.OrderItemsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.OrderItems{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateOrderItems")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateOrderItems: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "OrderItems",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateOrderItems: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("orders.order_items", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *OrderItemsService) UpdateOrderItemsMultiple(data []structures.OrderItemsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateOrderItemsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateOrderItemsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateOrderItemsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateOrderItemsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.OrderItems{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateOrderItemsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateOrderItemsMultiple", "not_found_error")
		}
...
func (s *OrderItemsService) CreateOrderItems(data structures.OrderItemsForm) (structures.OrderItemsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyOrderItemsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateOrderItems")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateOrderItems: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("orders.order_items", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *OrderItemsService) CreateOrderItemsMultiple(data []structures.OrderItemsForm) ([]structures.OrderItemsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateOrderItemsMultiple", "validation_error")
	}

	for i := range data {
		applyOrderItemsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateOrderItemsMultiple")
		}
		return nil
	})
...
func (s *OrderItemsService) DeleteOrderItems(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.OrderItems{}).Where("id = ?", id).Delete(&structures.OrderItems{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteOrderItems")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteOrderItems: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteOrderItems: %w", err)
		}

		go sseRuntime.Publish("orders.order_items", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *OrderItemsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructOrderItems(pathParams)
	if err != nil {
...
func buildWhereMapFromStructOrderItems(value interface{}) (map[string]interface{}, error) {
...
func (s *OrderItemsService) DeleteOrderItemsMultiple(identities []structures.OrderItemsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructOrderItems(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.OrderItems{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteOrderItemsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteOrderItemsMultiple", "not_found_error")
		}
