// OrdersService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for Orders resources with:
...
// IOrdersService defines the business logic interface for Orders operations.
//
// This interface abstracts all business operations for Orders resources, providing:
...
type IOrdersService interface {
	UpdateOrders(id types.URID, data structures.OrdersEdit) error
	UpdateOrdersMultiple(data []structures.OrdersBatchUpdate) error
	CreateOrders(data structures.OrdersForm) (structures.OrdersForm, error)
	CreateOrdersMultiple(data []structures.OrdersForm) ([]structures.OrdersForm, error)
	DeleteOrders(id types.URID) error
	DeleteOrdersMultiple(identities []structures.OrdersIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// OrdersService implements IOrdersService with enterprise-grade business logic.
//
...
//	})
func (s *OrdersService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *OrdersService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *OrdersService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyOrdersCreateDefaults(data *structures.OrdersForm) {
	if data.Currency == "" {
		data.Currency = "USD"
	}
}
...
func (s *OrdersService) UpdateOrders(id types.URID, data structures.OrdersEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Orders{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateOrders")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateOrders: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateOrders: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("orders.orders", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *OrdersService) UpdateOrdersMultiple(data []structures.OrdersBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateOrdersMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateOrdersMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateOrdersMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateOrdersMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.Orders{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateOrdersMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateOrdersMultiple", "not_found_error")
		}
...
func (s *OrdersService) CreateOrders(data structures.OrdersForm) (structures.OrdersForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyOrdersCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateOrders")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateOrders: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("orders.orders", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *OrdersService) CreateOrdersMultiple(data []structures.OrdersForm) ([]structures.OrdersForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateOrdersMultiple", "validation_error")
	}

	for i := range data {
		applyOrdersCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateOrdersMultiple")
		}
		return nil
	})
...
func (s *OrdersService) DeleteOrders(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Orders{}).Where("id = ?", id).Delete(&structures.Orders{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteOrders")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteOrders: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteOrders: %w", err)
		}

		go sseRuntime.Publish("orders.orders", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *OrdersService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructOrders(pathParams)
	if err != nil {
...
func buildWhereMapFromStructOrders(value interface{}) (map[string]interface{}, error) {
...
func (s *OrdersService) DeleteOrdersMultiple(identities []structures.OrdersIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructOrders(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.Orders{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteOrdersMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteOrdersMultiple", "not_found_error")
		}
