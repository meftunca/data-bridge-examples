...
var (
	createdUnreadNotificationsID string

	// Batch Update için PK'lar
	createdUnreadNotificationsIDForBatchUpdateA string
	createdUnreadNotificationsIDForBatchUpdateB string
	createdUnreadNotificationsIDNotBatched      string

	// Batch Delete için PK'lar
	createdUnreadNotificationsIDForBatchDelete1    string
	createdUnreadNotificationsIDForBatchDelete2    string
	createdUnreadNotificationsIDToKeepForBatchTest string
)

// extractUnreadNotificationsPKPath, create yanıtından veya pagination'dan tüm PK değerlerini
// alarak "/" ile birleştirilmiş bir path segment döndürür.
func extractUnreadNotificationsPKPath(t *testing.T, routePath, createBody string, pkFields []string) string {
...
// buildUnreadNotificationsBatchPKMap, "/" ile birleştirilmiş PK path'inden batch işlem
// için ayrı PK alanları içeren map oluşturur.
func buildUnreadNotificationsBatchPKMap(pkPath string, pkFields []string) map[string]interface{} {
...
// Test_D_ListWithPaginationUnreadNotifications: Kaynakları paginated olarak listeler.
func Test_D_ListWithPaginationUnreadNotifications(t *testing.T) {
	url := fmt.Sprintf("%s/pagination", "/analytics/unread-notifications")
