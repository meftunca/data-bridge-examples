// ReportExecutionsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for ReportExecutions resources with:
...
// IReportExecutionsService defines the business logic interface for ReportExecutions operations.
//
// This interface abstracts all business operations for ReportExecutions resources, providing:
...
type IReportExecutionsService interface {
	UpdateReportExecutions(id types.URID, data structures.ReportExecutionsEdit) error
	UpdateReportExecutionsMultiple(data []structures.ReportExecutionsBatchUpdate) error
	CreateReportExecutions(data structures.ReportExecutionsForm) (structures.ReportExecutionsForm, error)
	CreateReportExecutionsMultiple(data []structures.ReportExecutionsForm) ([]structures.ReportExecutionsForm, error)
	DeleteReportExecutions(id types.URID) error
	DeleteReportExecutionsMultiple(identities []structures.ReportExecutionsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// ReportExecutionsService implements IReportExecutionsService with enterprise-grade business logic.
//
...
type ReportExecutionsService struct {
...
//	})
func (s *ReportExecutionsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *ReportExecutionsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *ReportExecutionsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyReportExecutionsCreateDefaults(data *structures.ReportExecutionsForm) {
	if data.Status == "" {
		data.Status = "pending"
	}
}
...
func (s *ReportExecutionsService) UpdateReportExecutions(id types.URID, data structures.ReportExecutionsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.ReportExecutions{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateReportExecutions")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateReportExecutions: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "ReportExecutions",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateReportExecutions: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("analytics.report_executions", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *ReportExecutionsService) UpdateReportExecutionsMultiple(data []structures.ReportExecutionsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateReportExecutionsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateReportExecutionsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateReportExecutionsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateReportExecutionsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.ReportExecutions{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateReportExecutionsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateReportExecutionsMultiple", "not_found_error")
		}
...
func (s *ReportExecutionsService) CreateReportExecutions(data structures.ReportExecutionsForm) (structures.ReportExecutionsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyReportExecutionsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateReportExecutions")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateReportExecutions: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("analytics.report_executions", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *ReportExecutionsService) CreateReportExecutionsMultiple(data []structures.ReportExecutionsForm) ([]structures.ReportExecutionsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateReportExecutionsMultiple", "validation_error")
	}

	for i := range data {
		applyReportExecutionsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateReportExecutionsMultiple")
		}
		return nil
	})
...
func (s *ReportExecutionsService) DeleteReportExecutions(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.ReportExecutions{}).Where("id = ?", id).Delete(&structures.ReportExecutions{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteReportExecutions")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteReportExecutions: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteReportExecutions: %w", err)
		}

		go sseRuntime.Publish("analytics.report_executions", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *ReportExecutionsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructReportExecutions(pathParams)
	if err != nil {
...
func buildWhereMapFromStructReportExecutions(value interface{}) (map[string]interface{}, error) {
...
func (s *ReportExecutionsService) DeleteReportExecutionsMultiple(identities []structures.ReportExecutionsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructReportExecutions(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.ReportExecutions{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteReportExecutionsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteReportExecutionsMultiple", "not_found_error")
		}
