// MetricsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for Metrics resources with:
...
// IMetricsService defines the business logic interface for Metrics operations.
//
// This interface abstracts all business operations for Metrics resources, providing:
...
type IMetricsService interface {
	UpdateMetrics(id types.URID, data structures.MetricsEdit) error
	UpdateMetricsMultiple(data []structures.MetricsBatchUpdate) error
	CreateMetrics(data structures.MetricsForm) (structures.MetricsForm, error)
	CreateMetricsMultiple(data []structures.MetricsForm) ([]structures.MetricsForm, error)
	DeleteMetrics(id types.URID) error
	DeleteMetricsMultiple(identities []structures.MetricsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// MetricsService implements IMetricsService with enterprise-grade business logic.
//
...
//	})
func (s *MetricsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *MetricsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *MetricsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyMetricsCreateDefaults(data *structures.MetricsForm) {
	if !data.RecordedAt.Valid {
		data.RecordedAt = types.NullTime{Time: time.Now(), Valid: true}
	}
}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *MetricsService) UpdateMetrics(id types.URID, data structures.MetricsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Metrics{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateMetrics")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateMetrics: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateMetrics: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("analytics.metrics", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *MetricsService) UpdateMetricsMultiple(data []structures.MetricsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateMetricsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateMetricsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateMetricsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateMetricsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.Metrics{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateMetricsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateMetricsMultiple", "not_found_error")
		}
...
func (s *MetricsService) CreateMetrics(data structures.MetricsForm) (structures.MetricsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyMetricsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateMetrics")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateMetrics: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("analytics.metrics", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *MetricsService) CreateMetricsMultiple(data []structures.MetricsForm) ([]structures.MetricsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateMetricsMultiple", "validation_error")
	}

	for i := range data {
		applyMetricsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateMetricsMultiple")
		}
		return nil
	})
...
func (s *MetricsService) DeleteMetrics(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Metrics{}).Where("id = ?", id).Delete(&structures.Metrics{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteMetrics")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteMetrics: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteMetrics: %w", err)
		}

		go sseRuntime.Publish("analytics.metrics", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *MetricsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructMetrics(pathParams)
	if err != nil {
...
func buildWhereMapFromStructMetrics(value interface{}) (map[string]interface{}, error) {
...
func (s *MetricsService) DeleteMetricsMultiple(identities []structures.MetricsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructMetrics(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.Metrics{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteMetricsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteMetricsMultiple", "not_found_error")
		}
