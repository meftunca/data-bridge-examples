// NotificationsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for Notifications resources with:
...
// INotificationsService defines the business logic interface for Notifications operations.
//
// This interface abstracts all business operations for Notifications resources, providing:
...
type INotificationsService interface {
	UpdateNotifications(id types.URID, data structures.NotificationsEdit) error
	UpdateNotificationsMultiple(data []structures.NotificationsBatchUpdate) error
	CreateNotifications(data structures.NotificationsForm) (structures.NotificationsForm, error)
	CreateNotificationsMultiple(data []structures.NotificationsForm) ([]structures.NotificationsForm, error)
	DeleteNotifications(id types.URID) error
	DeleteNotificationsMultiple(identities []structures.NotificationsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// NotificationsService implements INotificationsService with enterprise-grade business logic.
//
...
//	})
func (s *NotificationsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *NotificationsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *NotificationsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyNotificationsCreateDefaults(data *structures.NotificationsForm) {
	if data.Channel == "" {
		data.Channel = "in_app"
	}
}
...
func (s *NotificationsService) UpdateNotifications(id types.URID, data structures.NotificationsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Notifications{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateNotifications")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateNotifications: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateNotifications: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("analytics.notifications", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *NotificationsService) UpdateNotificationsMultiple(data []structures.NotificationsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateNotificationsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateNotificationsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateNotificationsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateNotificationsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.Notifications{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateNotificationsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateNotificationsMultiple", "not_found_error")
		}
...
func (s *NotificationsService) CreateNotifications(data structures.NotificationsForm) (structures.NotificationsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyNotificationsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateNotifications")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateNotifications: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("analytics.notifications", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *NotificationsService) CreateNotificationsMultiple(data []structures.NotificationsForm) ([]structures.NotificationsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateNotificationsMultiple", "validation_error")
	}

	for i := range data {
		applyNotificationsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateNotificationsMultiple")
		}
		return nil
	})
...
func (s *NotificationsService) DeleteNotifications(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Notifications{}).Where("id = ?", id).Delete(&structures.Notifications{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteNotifications")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteNotifications: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteNotifications: %w", err)
		}

		go sseRuntime.Publish("analytics.notifications", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *NotificationsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructNotifications(pathParams)
	if err != nil {
...
func buildWhereMapFromStructNotifications(value interface{}) (map[string]interface{}, error) {
...
func (s *NotificationsService) DeleteNotificationsMultiple(identities []structures.NotificationsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructNotifications(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.Notifications{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteNotificationsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteNotificationsMultiple", "not_found_error")
		}
