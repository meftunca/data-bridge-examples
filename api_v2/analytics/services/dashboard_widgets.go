// DashboardWidgetsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for DashboardWidgets resources with:
...
// IDashboardWidgetsService defines the business logic interface for DashboardWidgets operations.
//
// This interface abstracts all business operations for DashboardWidgets resources, providing:
...
type IDashboardWidgetsService interface {
	UpdateDashboardWidgets(id types.URID, data structures.DashboardWidgetsEdit) error
	UpdateDashboardWidgetsMultiple(data []structures.DashboardWidgetsBatchUpdate) error
	CreateDashboardWidgets(data structures.DashboardWidgetsForm) (structures.DashboardWidgetsForm, error)
	CreateDashboardWidgetsMultiple(data []structures.DashboardWidgetsForm) ([]structures.DashboardWidgetsForm, error)
	DeleteDashboardWidgets(id types.URID) error
	DeleteDashboardWidgetsMultiple(identities []structures.DashboardWidgetsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// DashboardWidgetsService implements IDashboardWidgetsService with enterprise-grade business logic.
//
...
type DashboardWidgetsService struct {
...
//	})
func (s *DashboardWidgetsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *DashboardWidgetsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *DashboardWidgetsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyDashboardWidgetsCreateDefaults(data *structures.DashboardWidgetsForm) {
	if data.WidgetType == "" {
...
func (s *DashboardWidgetsService) UpdateDashboardWidgets(id types.URID, data structures.DashboardWidgetsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.DashboardWidgets{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateDashboardWidgets")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateDashboardWidgets: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "DashboardWidgets",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateDashboardWidgets: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("analytics.dashboard_widgets", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *DashboardWidgetsService) UpdateDashboardWidgetsMultiple(data []structures.DashboardWidgetsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateDashboardWidgetsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateDashboardWidgetsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateDashboardWidgetsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateDashboardWidgetsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.DashboardWidgets{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateDashboardWidgetsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateDashboardWidgetsMultiple", "not_found_error")
		}
...
func (s *DashboardWidgetsService) CreateDashboardWidgets(data structures.DashboardWidgetsForm) (structures.DashboardWidgetsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyDashboardWidgetsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateDashboardWidgets")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateDashboardWidgets: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("analytics.dashboard_widgets", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *DashboardWidgetsService) CreateDashboardWidgetsMultiple(data []structures.DashboardWidgetsForm) ([]structures.DashboardWidgetsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateDashboardWidgetsMultiple", "validation_error")
	}

	for i := range data {
		applyDashboardWidgetsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateDashboardWidgetsMultiple")
		}
		return nil
	})
...
func (s *DashboardWidgetsService) DeleteDashboardWidgets(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.DashboardWidgets{}).Where("id = ?", id).Delete(&structures.DashboardWidgets{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteDashboardWidgets")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteDashboardWidgets: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteDashboardWidgets: %w", err)
		}

		go sseRuntime.Publish("analytics.dashboard_widgets", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *DashboardWidgetsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructDashboardWidgets(pathParams)
	if err != nil {
...
func buildWhereMapFromStructDashboardWidgets(value interface{}) (map[string]interface{}, error) {
...
func (s *DashboardWidgetsService) DeleteDashboardWidgetsMultiple(identities []structures.DashboardWidgetsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructDashboardWidgets(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.DashboardWidgets{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteDashboardWidgetsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteDashboardWidgetsMultiple", "not_found_error")
		}
