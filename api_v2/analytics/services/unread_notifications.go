// UnreadNotificationsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for UnreadNotifications resources with:
...
// IUnreadNotificationsService defines the business logic interface for UnreadNotifications operations.
//
// This interface abstracts all business operations for UnreadNotifications resources, providing:
...
type IUnreadNotificationsService interface {
	// GetUnreadNotifications(filter structures.UnreadNotificationsFilter) ([]structures.UnreadNotifications, error) // Pagination ile yönetildiği için genellikle gerekmez
}

// UnreadNotificationsService implements IUnreadNotificationsService with enterprise-grade business logic.
//
...
type UnreadNotificationsService struct {
...
//	})
func (s *UnreadNotificationsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *UnreadNotificationsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *UnreadNotificationsService) validateContext(ctx context.Context) error {
	if ctx == nil {
