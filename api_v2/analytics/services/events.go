// EventsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for Events resources with:
...
// IEventsService defines the business logic interface for Events operations.
//
// This interface abstracts all business operations for Events resources, providing:
...
type IEventsService interface {
	UpdateEvents(id types.URID, data structures.EventsEdit) error
	UpdateEventsMultiple(data []structures.EventsBatchUpdate) error
	CreateEvents(data structures.EventsForm) (structures.EventsForm, error)
	CreateEventsMultiple(data []structures.EventsForm) ([]structures.EventsForm, error)
	DeleteEvents(id types.URID) error
	DeleteEventsMultiple(identities []structures.EventsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// EventsService implements IEventsService with enterprise-grade business logic.
//
...
//	})
func (s *EventsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *EventsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *EventsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyEventsCreateDefaults(data *structures.EventsForm) {

}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *EventsService) UpdateEvents(id types.URID, data structures.EventsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Events{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateEvents")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateEvents: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateEvents: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("analytics.events", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *EventsService) UpdateEventsMultiple(data []structures.EventsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateEventsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateEventsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateEventsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateEventsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.Events{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateEventsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateEventsMultiple", "not_found_error")
		}
...
func (s *EventsService) CreateEvents(data structures.EventsForm) (structures.EventsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyEventsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateEvents")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateEvents: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("analytics.events", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *EventsService) CreateEventsMultiple(data []structures.EventsForm) ([]structures.EventsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateEventsMultiple", "validation_error")
	}

	for i := range data {
		applyEventsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateEventsMultiple")
		}
		return nil
	})
...
func (s *EventsService) DeleteEvents(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Events{}).Where("id = ?", id).Delete(&structures.Events{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteEvents")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteEvents: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteEvents: %w", err)
		}

		go sseRuntime.Publish("analytics.events", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *EventsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructEvents(pathParams)
	if err != nil {
...
func buildWhereMapFromStructEvents(value interface{}) (map[string]interface{}, error) {
...
func (s *EventsService) DeleteEventsMultiple(identities []structures.EventsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructEvents(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.Events{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteEventsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteEventsMultiple", "not_found_error")
		}
