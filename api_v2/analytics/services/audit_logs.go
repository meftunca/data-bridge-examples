// AuditLogsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for AuditLogs resources with:
...
// IAuditLogsService defines the business logic interface for AuditLogs operations.
//
// This interface abstracts all business operations for AuditLogs resources, providing:
...
type IAuditLogsService interface {
	UpdateAuditLogs(id types.URID, data structures.AuditLogsEdit) error
	UpdateAuditLogsMultiple(data []structures.AuditLogsBatchUpdate) error
	CreateAuditLogs(data structures.AuditLogsForm) (structures.AuditLogsForm, error)
	CreateAuditLogsMultiple(data []structures.AuditLogsForm) ([]structures.AuditLogsForm, error)
	DeleteAuditLogs(id types.URID) error
	DeleteAuditLogsMultiple(identities []structures.AuditLogsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// AuditLogsService implements IAuditLogsService with enterprise-grade business logic.
//
...
//	})
func (s *AuditLogsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *AuditLogsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *AuditLogsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyAuditLogsCreateDefaults(data *structures.AuditLogsForm) {

}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *AuditLogsService) UpdateAuditLogs(id types.URID, data structures.AuditLogsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.AuditLogs{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateAuditLogs")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateAuditLogs: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "AuditLogs",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateAuditLogs: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("analytics.audit_logs", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *AuditLogsService) UpdateAuditLogsMultiple(data []structures.AuditLogsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateAuditLogsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateAuditLogsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateAuditLogsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateAuditLogsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.AuditLogs{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateAuditLogsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateAuditLogsMultiple", "not_found_error")
		}
...
func (s *AuditLogsService) CreateAuditLogs(data structures.AuditLogsForm) (structures.AuditLogsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyAuditLogsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateAuditLogs")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateAuditLogs: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("analytics.audit_logs", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *AuditLogsService) CreateAuditLogsMultiple(data []structures.AuditLogsForm) ([]structures.AuditLogsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateAuditLogsMultiple", "validation_error")
	}

	for i := range data {
		applyAuditLogsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateAuditLogsMultiple")
		}
		return nil
	})
...
func (s *AuditLogsService) DeleteAuditLogs(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.AuditLogs{}).Where("id = ?", id).Delete(&structures.AuditLogs{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteAuditLogs")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteAuditLogs: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteAuditLogs: %w", err)
		}

		go sseRuntime.Publish("analytics.audit_logs", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *AuditLogsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructAuditLogs(pathParams)
	if err != nil {
...
func buildWhereMapFromStructAuditLogs(value interface{}) (map[string]interface{}, error) {
...
func (s *AuditLogsService) DeleteAuditLogsMultiple(identities []structures.AuditLogsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructAuditLogs(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.AuditLogs{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteAuditLogsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteAuditLogsMultiple", "not_found_error")
		}
