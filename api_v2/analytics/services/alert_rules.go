// AlertRulesService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for AlertRules resources with:
...
// IAlertRulesService defines the business logic interface for AlertRules operations.
//
// This interface abstracts all business operations for AlertRules resources, providing:
...
type IAlertRulesService interface {
	UpdateAlertRules(id types.URID, data structures.AlertRulesEdit) error
	UpdateAlertRulesMultiple(data []structures.AlertRulesBatchUpdate) error
	CreateAlertRules(data structures.AlertRulesForm) (structures.AlertRulesForm, error)
	CreateAlertRulesMultiple(data []structures.AlertRulesForm) ([]structures.AlertRulesForm, error)
	DeleteAlertRules(id types.URID) error
	DeleteAlertRulesMultiple(identities []structures.AlertRulesIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// AlertRulesService implements IAlertRulesService with enterprise-grade business logic.
//
...
//	})
func (s *AlertRulesService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *AlertRulesService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *AlertRulesService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyAlertRulesCreateDefaults(data *structures.AlertRulesForm) {
...
func (s *AlertRulesService) UpdateAlertRules(id types.URID, data structures.AlertRulesEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.AlertRules{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateAlertRules")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateAlertRules: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "AlertRules",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateAlertRules: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("analytics.alert_rules", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *AlertRulesService) UpdateAlertRulesMultiple(data []structures.AlertRulesBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateAlertRulesMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateAlertRulesMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateAlertRulesMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateAlertRulesMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.AlertRules{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateAlertRulesMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateAlertRulesMultiple", "not_found_error")
		}
...
func (s *AlertRulesService) CreateAlertRules(data structures.AlertRulesForm) (structures.AlertRulesForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyAlertRulesCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateAlertRules")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateAlertRules: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("analytics.alert_rules", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *AlertRulesService) CreateAlertRulesMultiple(data []structures.AlertRulesForm) ([]structures.AlertRulesForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateAlertRulesMultiple", "validation_error")
	}

	for i := range data {
		applyAlertRulesCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateAlertRulesMultiple")
		}
		return nil
	})
...
func (s *AlertRulesService) DeleteAlertRules(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.AlertRules{}).Where("id = ?", id).Delete(&structures.AlertRules{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteAlertRules")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteAlertRules: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteAlertRules: %w", err)
		}

		go sseRuntime.Publish("analytics.alert_rules", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *AlertRulesService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructAlertRules(pathParams)
	if err != nil {
...
func buildWhereMapFromStructAlertRules(value interface{}) (map[string]interface{}, error) {
...
func (s *AlertRulesService) DeleteAlertRulesMultiple(identities []structures.AlertRulesIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructAlertRules(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.AlertRules{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteAlertRulesMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteAlertRulesMultiple", "not_found_error")
		}
