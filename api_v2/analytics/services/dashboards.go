// DashboardsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for Dashboards resources with:
...
// IDashboardsService defines the business logic interface for Dashboards operations.
//
// This interface abstracts all business operations for Dashboards resources, providing:
...
type IDashboardsService interface {
	UpdateDashboards(id types.URID, data structures.DashboardsEdit) error
	UpdateDashboardsMultiple(data []structures.DashboardsBatchUpdate) error
	CreateDashboards(data structures.DashboardsForm) (structures.DashboardsForm, error)
	CreateDashboardsMultiple(data []structures.DashboardsForm) ([]structures.DashboardsForm, error)
	DeleteDashboards(id types.URID) error
	DeleteDashboardsMultiple(identities []structures.DashboardsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// DashboardsService implements IDashboardsService with enterprise-grade business logic.
//
...
//	})
func (s *DashboardsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *DashboardsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *DashboardsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyDashboardsCreateDefaults(data *structures.DashboardsForm) {

}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *DashboardsService) UpdateDashboards(id types.URID, data structures.DashboardsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Dashboards{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateDashboards")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateDashboards: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "Dashboards",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateDashboards: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("analytics.dashboards", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *DashboardsService) UpdateDashboardsMultiple(data []structures.DashboardsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateDashboardsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateDashboardsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateDashboardsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateDashboardsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.Dashboards{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateDashboardsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateDashboardsMultiple", "not_found_error")
		}
...
func (s *DashboardsService) CreateDashboards(data structures.DashboardsForm) (structures.DashboardsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyDashboardsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateDashboards")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateDashboards: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("analytics.dashboards", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *DashboardsService) CreateDashboardsMultiple(data []structures.DashboardsForm) ([]structures.DashboardsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateDashboardsMultiple", "validation_error")
	}

	for i := range data {
		applyDashboardsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateDashboardsMultiple")
		}
		return nil
	})
...
func (s *DashboardsService) DeleteDashboards(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Dashboards{}).Where("id = ?", id).Delete(&structures.Dashboards{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteDashboards")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteDashboards: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteDashboards: %w", err)
		}

		go sseRuntime.Publish("analytics.dashboards", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *DashboardsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructDashboards(pathParams)
	if err != nil {
...
func buildWhereMapFromStructDashboards(value interface{}) (map[string]interface{}, error) {
...
func (s *DashboardsService) DeleteDashboardsMultiple(identities []structures.DashboardsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructDashboards(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.Dashboards{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteDashboardsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteDashboardsMultiple", "not_found_error")
		}
