// AlertHistoryService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for AlertHistory resources with:
...
// IAlertHistoryService defines the business logic interface for AlertHistory operations.
//
// This interface abstracts all business operations for AlertHistory resources, providing:
...
type IAlertHistoryService interface {
	UpdateAlertHistory(id types.URID, data structures.AlertHistoryEdit) error
	UpdateAlertHistoryMultiple(data []structures.AlertHistoryBatchUpdate) error
	CreateAlertHistory(data structures.AlertHistoryForm) (structures.AlertHistoryForm, error)
	CreateAlertHistoryMultiple(data []structures.AlertHistoryForm) ([]structures.AlertHistoryForm, error)
	DeleteAlertHistory(id types.URID) error
	DeleteAlertHistoryMultiple(identities []structures.AlertHistoryIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// AlertHistoryService implements IAlertHistoryService with enterprise-grade business logic.
//
...
//	})
func (s *AlertHistoryService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *AlertHistoryService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *AlertHistoryService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyAlertHistoryCreateDefaults(data *structures.AlertHistoryForm) {

}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *AlertHistoryService) UpdateAlertHistory(id types.URID, data structures.AlertHistoryEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.AlertHistory{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateAlertHistory")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateAlertHistory: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "AlertHistory",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateAlertHistory: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("analytics.alert_history", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *AlertHistoryService) UpdateAlertHistoryMultiple(data []structures.AlertHistoryBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateAlertHistoryMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateAlertHistoryMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateAlertHistoryMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateAlertHistoryMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.AlertHistory{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateAlertHistoryMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateAlertHistoryMultiple", "not_found_error")
		}
...
func (s *AlertHistoryService) CreateAlertHistory(data structures.AlertHistoryForm) (structures.AlertHistoryForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyAlertHistoryCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateAlertHistory")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateAlertHistory: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("analytics.alert_history", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *AlertHistoryService) CreateAlertHistoryMultiple(data []structures.AlertHistoryForm) ([]structures.AlertHistoryForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateAlertHistoryMultiple", "validation_error")
	}

	for i := range data {
		applyAlertHistoryCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateAlertHistoryMultiple")
		}
		return nil
	})
...
func (s *AlertHistoryService) DeleteAlertHistory(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.AlertHistory{}).Where("id = ?", id).Delete(&structures.AlertHistory{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteAlertHistory")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteAlertHistory: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteAlertHistory: %w", err)
		}

		go sseRuntime.Publish("analytics.alert_history", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *AlertHistoryService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructAlertHistory(pathParams)
	if err != nil {
...
func buildWhereMapFromStructAlertHistory(value interface{}) (map[string]interface{}, error) {
...
func (s *AlertHistoryService) DeleteAlertHistoryMultiple(identities []structures.AlertHistoryIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructAlertHistory(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.AlertHistory{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteAlertHistoryMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteAlertHistoryMultiple", "not_found_error")
		}
