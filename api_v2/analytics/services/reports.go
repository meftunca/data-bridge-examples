// ReportsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for Reports resources with:
...
// IReportsService defines the business logic interface for Reports operations.
//
// This interface abstracts all business operations for Reports resources, providing:
...
type IReportsService interface {
	UpdateReports(id types.URID, data structures.ReportsEdit) error
	UpdateReportsMultiple(data []structures.ReportsBatchUpdate) error
	CreateReports(data structures.ReportsForm) (structures.ReportsForm, error)
	CreateReportsMultiple(data []structures.ReportsForm) ([]structures.ReportsForm, error)
	DeleteReports(id types.URID) error
	DeleteReportsMultiple(identities []structures.ReportsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// ReportsService implements IReportsService with enterprise-grade business logic.
//
...
//	})
func (s *ReportsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *ReportsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *ReportsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyReportsCreateDefaults(data *structures.ReportsForm) {

}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *ReportsService) UpdateReports(id types.URID, data structures.ReportsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Reports{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateReports")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateReports: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "Reports",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateReports: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("analytics.reports", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *ReportsService) UpdateReportsMultiple(data []structures.ReportsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateReportsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateReportsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateReportsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateReportsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.Reports{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateReportsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateReportsMultiple", "not_found_error")
		}
...
func (s *ReportsService) CreateReports(data structures.ReportsForm) (structures.ReportsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyReportsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateReports")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateReports: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("analytics.reports", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *ReportsService) CreateReportsMultiple(data []structures.ReportsForm) ([]structures.ReportsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateReportsMultiple", "validation_error")
	}

	for i := range data {
		applyReportsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateReportsMultiple")
		}
		return nil
	})
...
func (s *ReportsService) DeleteReports(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Reports{}).Where("id = ?", id).Delete(&structures.Reports{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteReports")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteReports: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteReports: %w", err)
		}

		go sseRuntime.Publish("analytics.reports", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *ReportsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructReports(pathParams)
	if err != nil {
...
func buildWhereMapFromStructReports(value interface{}) (map[string]interface{}, error) {
...
func (s *ReportsService) DeleteReportsMultiple(identities []structures.ReportsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructReports(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.Reports{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteReportsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteReportsMultiple", "not_found_error")
		}
