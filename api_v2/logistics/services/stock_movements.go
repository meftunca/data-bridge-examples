// StockMovementsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for StockMovements resources with:
...
// IStockMovementsService defines the business logic interface for StockMovements operations.
//
// This interface abstracts all business operations for StockMovements resources, providing:
...
type IStockMovementsService interface {
	UpdateStockMovements(id types.URID, data structures.StockMovementsEdit) error
	UpdateStockMovementsMultiple(data []structures.StockMovementsBatchUpdate) error
	CreateStockMovements(data structures.StockMovementsForm) (structures.StockMovementsForm, error)
	CreateStockMovementsMultiple(data []structures.StockMovementsForm) ([]structures.StockMovementsForm, error)
	DeleteStockMovements(id types.URID) error
	DeleteStockMovementsMultiple(identities []structures.StockMovementsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// StockMovementsService implements IStockMovementsService with enterprise-grade business logic.
//
...
//	})
func (s *StockMovementsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *StockMovementsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *StockMovementsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyStockMovementsCreateDefaults(data *structures.StockMovementsForm) {
...
func (s *StockMovementsService) UpdateStockMovements(id types.URID, data structures.StockMovementsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.StockMovements{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateStockMovements")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateStockMovements: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "StockMovements",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateStockMovements: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("logistics.stock_movements", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *StockMovementsService) UpdateStockMovementsMultiple(data []structures.StockMovementsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateStockMovementsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateStockMovementsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateStockMovementsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateStockMovementsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.StockMovements{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateStockMovementsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateStockMovementsMultiple", "not_found_error")
		}
...
func (s *StockMovementsService) CreateStockMovements(data structures.StockMovementsForm) (structures.StockMovementsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyStockMovementsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateStockMovements")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateStockMovements: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("logistics.stock_movements", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *StockMovementsService) CreateStockMovementsMultiple(data []structures.StockMovementsForm) ([]structures.StockMovementsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateStockMovementsMultiple", "validation_error")
	}

	for i := range data {
		applyStockMovementsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateStockMovementsMultiple")
		}
		return nil
	})
...
func (s *StockMovementsService) DeleteStockMovements(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.StockMovements{}).Where("id = ?", id).Delete(&structures.StockMovements{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteStockMovements")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteStockMovements: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteStockMovements: %w", err)
		}

		go sseRuntime.Publish("logistics.stock_movements", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *StockMovementsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructStockMovements(pathParams)
	if err != nil {
...
func buildWhereMapFromStructStockMovements(value interface{}) (map[string]interface{}, error) {
...
func (s *StockMovementsService) DeleteStockMovementsMultiple(identities []structures.StockMovementsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructStockMovements(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.StockMovements{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteStockMovementsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteStockMovementsMultiple", "not_found_error")
		}
