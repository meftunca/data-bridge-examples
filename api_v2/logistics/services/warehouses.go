// WarehousesService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for Warehouses resources with:
...
// IWarehousesService defines the business logic interface for Warehouses operations.
//
// This interface abstracts all business operations for Warehouses resources, providing:
...
type IWarehousesService interface {
	UpdateWarehouses(id types.URID, data structures.WarehousesEdit) error
	UpdateWarehousesMultiple(data []structures.WarehousesBatchUpdate) error
	CreateWarehouses(data structures.WarehousesForm) (structures.WarehousesForm, error)
	CreateWarehousesMultiple(data []structures.WarehousesForm) ([]structures.WarehousesForm, error)
	DeleteWarehouses(id types.URID) error
	DeleteWarehousesMultiple(identities []structures.WarehousesIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// WarehousesService implements IWarehousesService with enterprise-grade business logic.
//
...
//	})
func (s *WarehousesService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *WarehousesService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *WarehousesService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyWarehousesCreateDefaults(data *structures.WarehousesForm) {

}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *WarehousesService) UpdateWarehouses(id types.URID, data structures.WarehousesEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Warehouses{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateWarehouses")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateWarehouses: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "Warehouses",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateWarehouses: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("logistics.warehouses", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *WarehousesService) UpdateWarehousesMultiple(data []structures.WarehousesBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateWarehousesMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateWarehousesMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateWarehousesMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateWarehousesMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.Warehouses{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateWarehousesMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateWarehousesMultiple", "not_found_error")
		}
...
func (s *WarehousesService) CreateWarehouses(data structures.WarehousesForm) (structures.WarehousesForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyWarehousesCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateWarehouses")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateWarehouses: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("logistics.warehouses", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *WarehousesService) CreateWarehousesMultiple(data []structures.WarehousesForm) ([]structures.WarehousesForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateWarehousesMultiple", "validation_error")
	}

	for i := range data {
		applyWarehousesCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateWarehousesMultiple")
		}
		return nil
	})
...
func (s *WarehousesService) DeleteWarehouses(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Warehouses{}).Where("id = ?", id).Delete(&structures.Warehouses{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteWarehouses")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteWarehouses: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteWarehouses: %w", err)
		}

		go sseRuntime.Publish("logistics.warehouses", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *WarehousesService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructWarehouses(pathParams)
	if err != nil {
...
func buildWhereMapFromStructWarehouses(value interface{}) (map[string]interface{}, error) {
...
func (s *WarehousesService) DeleteWarehousesMultiple(identities []structures.WarehousesIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructWarehouses(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.Warehouses{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteWarehousesMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteWarehousesMultiple", "not_found_error")
		}
