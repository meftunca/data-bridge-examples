// ShipmentItemsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for ShipmentItems resources with:
...
// IShipmentItemsService defines the business logic interface for ShipmentItems operations.
//
// This interface abstracts all business operations for ShipmentItems resources, providing:
...
type IShipmentItemsService interface {
	UpdateShipmentItems(id types.URID, data structures.ShipmentItemsEdit) error
	UpdateShipmentItemsMultiple(data []structures.ShipmentItemsBatchUpdate) error
	CreateShipmentItems(data structures.ShipmentItemsForm) (structures.ShipmentItemsForm, error)
	CreateShipmentItemsMultiple(data []structures.ShipmentItemsForm) ([]structures.ShipmentItemsForm, error)
	DeleteShipmentItems(id types.URID) error
	DeleteShipmentItemsMultiple(identities []structures.ShipmentItemsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// ShipmentItemsService implements IShipmentItemsService with enterprise-grade business logic.
//
...
type ShipmentItemsService struct {
...
//	})
func (s *ShipmentItemsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *ShipmentItemsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *ShipmentItemsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyShipmentItemsCreateDefaults(data *structures.ShipmentItemsForm) {
	if data.Quantity == 0 {
		data.Quantity = 1
	}
}
...
func (s *ShipmentItemsService) UpdateShipmentItems(id types.URID, data structures.ShipmentItemsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.ShipmentItems{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateShipmentItems")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateShipmentItems: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "ShipmentItems",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateShipmentItems: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("logistics.shipment_items", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *ShipmentItemsService) UpdateShipmentItemsMultiple(data []structures.ShipmentItemsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateShipmentItemsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateShipmentItemsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateShipmentItemsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateShipmentItemsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.ShipmentItems{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateShipmentItemsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateShipmentItemsMultiple", "not_found_error")
		}
...
func (s *ShipmentItemsService) CreateShipmentItems(data structures.ShipmentItemsForm) (structures.ShipmentItemsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyShipmentItemsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateShipmentItems")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateShipmentItems: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("logistics.shipment_items", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *ShipmentItemsService) CreateShipmentItemsMultiple(data []structures.ShipmentItemsForm) ([]structures.ShipmentItemsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateShipmentItemsMultiple", "validation_error")
	}

	for i := range data {
		applyShipmentItemsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateShipmentItemsMultiple")
		}
		return nil
	})
...
func (s *ShipmentItemsService) DeleteShipmentItems(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.ShipmentItems{}).Where("id = ?", id).Delete(&structures.ShipmentItems{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteShipmentItems")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteShipmentItems: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteShipmentItems: %w", err)
		}

		go sseRuntime.Publish("logistics.shipment_items", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *ShipmentItemsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructShipmentItems(pathParams)
	if err != nil {
...
func buildWhereMapFromStructShipmentItems(value interface{}) (map[string]interface{}, error) {
...
func (s *ShipmentItemsService) DeleteShipmentItemsMultiple(identities []structures.ShipmentItemsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructShipmentItems(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.ShipmentItems{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteShipmentItemsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteShipmentItemsMultiple", "not_found_error")
		}
