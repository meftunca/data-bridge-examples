// PurchaseOrderItemsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for PurchaseOrderItems resources with:
...
// IPurchaseOrderItemsService defines the business logic interface for PurchaseOrderItems operations.
//
// This interface abstracts all business operations for PurchaseOrderItems resources, providing:
...
type IPurchaseOrderItemsService interface {
	UpdatePurchaseOrderItems(id types.URID, data structures.PurchaseOrderItemsEdit) error
	UpdatePurchaseOrderItemsMultiple(data []structures.PurchaseOrderItemsBatchUpdate) error
	CreatePurchaseOrderItems(data structures.PurchaseOrderItemsForm) (structures.PurchaseOrderItemsForm, error)
	CreatePurchaseOrderItemsMultiple(data []structures.PurchaseOrderItemsForm) ([]structures.PurchaseOrderItemsForm, error)
	DeletePurchaseOrderItems(id types.URID) error
	DeletePurchaseOrderItemsMultiple(identities []structures.PurchaseOrderItemsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// PurchaseOrderItemsService implements IPurchaseOrderItemsService with enterprise-grade business logic.
//
...
type PurchaseOrderItemsService struct {
...
//	})
func (s *PurchaseOrderItemsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *PurchaseOrderItemsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *PurchaseOrderItemsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyPurchaseOrderItemsCreateDefaults(data *structures.PurchaseOrderItemsForm) {
	if data.Quantity == 0 {
		data.Quantity = 1
	}
}
...
func (s *PurchaseOrderItemsService) UpdatePurchaseOrderItems(id types.URID, data structures.PurchaseOrderItemsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.PurchaseOrderItems{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdatePurchaseOrderItems")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdatePurchaseOrderItems: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "PurchaseOrderItems",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdatePurchaseOrderItems: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("logistics.purchase_order_items", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *PurchaseOrderItemsService) UpdatePurchaseOrderItemsMultiple(data []structures.PurchaseOrderItemsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdatePurchaseOrderItemsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdatePurchaseOrderItemsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdatePurchaseOrderItemsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdatePurchaseOrderItemsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.PurchaseOrderItems{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdatePurchaseOrderItemsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdatePurchaseOrderItemsMultiple", "not_found_error")
		}
...
func (s *PurchaseOrderItemsService) CreatePurchaseOrderItems(data structures.PurchaseOrderItemsForm) (structures.PurchaseOrderItemsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyPurchaseOrderItemsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreatePurchaseOrderItems")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreatePurchaseOrderItems: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("logistics.purchase_order_items", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *PurchaseOrderItemsService) CreatePurchaseOrderItemsMultiple(data []structures.PurchaseOrderItemsForm) ([]structures.PurchaseOrderItemsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreatePurchaseOrderItemsMultiple", "validation_error")
	}

	for i := range data {
		applyPurchaseOrderItemsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreatePurchaseOrderItemsMultiple")
		}
		return nil
	})
...
func (s *PurchaseOrderItemsService) DeletePurchaseOrderItems(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.PurchaseOrderItems{}).Where("id = ?", id).Delete(&structures.PurchaseOrderItems{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeletePurchaseOrderItems")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeletePurchaseOrderItems: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeletePurchaseOrderItems: %w", err)
		}

		go sseRuntime.Publish("logistics.purchase_order_items", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *PurchaseOrderItemsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructPurchaseOrderItems(pathParams)
	if err != nil {
...
func buildWhereMapFromStructPurchaseOrderItems(value interface{}) (map[string]interface{}, error) {
...
func (s *PurchaseOrderItemsService) DeletePurchaseOrderItemsMultiple(identities []structures.PurchaseOrderItemsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructPurchaseOrderItems(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.PurchaseOrderItems{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeletePurchaseOrderItemsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeletePurchaseOrderItemsMultiple", "not_found_error")
		}
