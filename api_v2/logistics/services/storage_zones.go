// StorageZonesService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for StorageZones resources with:
...
// IStorageZonesService defines the business logic interface for StorageZones operations.
//
// This interface abstracts all business operations for StorageZones resources, providing:
...
type IStorageZonesService interface {
	UpdateStorageZones(id types.URID, data structures.StorageZonesEdit) error
	UpdateStorageZonesMultiple(data []structures.StorageZonesBatchUpdate) error
	CreateStorageZones(data structures.StorageZonesForm) (structures.StorageZonesForm, error)
	CreateStorageZonesMultiple(data []structures.StorageZonesForm) ([]structures.StorageZonesForm, error)
	DeleteStorageZones(id types.URID) error
	DeleteStorageZonesMultiple(identities []structures.StorageZonesIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// StorageZonesService implements IStorageZonesService with enterprise-grade business logic.
//
...
type StorageZonesService struct {
...
//	})
func (s *StorageZonesService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *StorageZonesService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *StorageZonesService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyStorageZonesCreateDefaults(data *structures.StorageZonesForm) {
	if data.ZoneCode == "" {
		data.ZoneCode = "A"
	}
	if data.ZoneType == "" {
		data.ZoneType = "general"
	}
}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *StorageZonesService) UpdateStorageZones(id types.URID, data structures.StorageZonesEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.StorageZones{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateStorageZones")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateStorageZones: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "StorageZones",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateStorageZones: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("logistics.storage_zones", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *StorageZonesService) UpdateStorageZonesMultiple(data []structures.StorageZonesBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateStorageZonesMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateStorageZonesMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateStorageZonesMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateStorageZonesMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.StorageZones{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateStorageZonesMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateStorageZonesMultiple", "not_found_error")
		}
...
func (s *StorageZonesService) CreateStorageZones(data structures.StorageZonesForm) (structures.StorageZonesForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyStorageZonesCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateStorageZones")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateStorageZones: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("logistics.storage_zones", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *StorageZonesService) CreateStorageZonesMultiple(data []structures.StorageZonesForm) ([]structures.StorageZonesForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateStorageZonesMultiple", "validation_error")
	}

	for i := range data {
		applyStorageZonesCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateStorageZonesMultiple")
		}
		return nil
	})
...
func (s *StorageZonesService) DeleteStorageZones(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.StorageZones{}).Where("id = ?", id).Delete(&structures.StorageZones{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteStorageZones")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteStorageZones: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteStorageZones: %w", err)
		}

		go sseRuntime.Publish("logistics.storage_zones", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *StorageZonesService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructStorageZones(pathParams)
	if err != nil {
...
func buildWhereMapFromStructStorageZones(value interface{}) (map[string]interface{}, error) {
...
func (s *StorageZonesService) DeleteStorageZonesMultiple(identities []structures.StorageZonesIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructStorageZones(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.StorageZones{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteStorageZonesMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteStorageZonesMultiple", "not_found_error")
		}
