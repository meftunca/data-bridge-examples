// StorageBinsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for StorageBins resources with:
...
// IStorageBinsService defines the business logic interface for StorageBins operations.
//
// This interface abstracts all business operations for StorageBins resources, providing:
...
type IStorageBinsService interface {
	UpdateStorageBins(id types.URID, data structures.StorageBinsEdit) error
	UpdateStorageBinsMultiple(data []structures.StorageBinsBatchUpdate) error
	CreateStorageBins(data structures.StorageBinsForm) (structures.StorageBinsForm, error)
	CreateStorageBinsMultiple(data []structures.StorageBinsForm) ([]structures.StorageBinsForm, error)
	DeleteStorageBins(id types.URID) error
	DeleteStorageBinsMultiple(identities []structures.StorageBinsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// StorageBinsService implements IStorageBinsService with enterprise-grade business logic.
//
...
type StorageBinsService struct {
...
//	})
func (s *StorageBinsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *StorageBinsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *StorageBinsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyStorageBinsCreateDefaults(data *structures.StorageBinsForm) {
	if data.BinCode == "" {
		data.BinCode = "B001"
	}
...
func (s *StorageBinsService) UpdateStorageBins(id types.URID, data structures.StorageBinsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.StorageBins{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateStorageBins")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateStorageBins: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "StorageBins",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateStorageBins: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("logistics.storage_bins", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *StorageBinsService) UpdateStorageBinsMultiple(data []structures.StorageBinsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateStorageBinsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateStorageBinsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateStorageBinsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateStorageBinsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.StorageBins{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateStorageBinsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateStorageBinsMultiple", "not_found_error")
		}
...
func (s *StorageBinsService) CreateStorageBins(data structures.StorageBinsForm) (structures.StorageBinsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyStorageBinsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateStorageBins")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateStorageBins: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("logistics.storage_bins", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *StorageBinsService) CreateStorageBinsMultiple(data []structures.StorageBinsForm) ([]structures.StorageBinsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateStorageBinsMultiple", "validation_error")
	}

	for i := range data {
		applyStorageBinsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateStorageBinsMultiple")
		}
		return nil
	})
...
func (s *StorageBinsService) DeleteStorageBins(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.StorageBins{}).Where("id = ?", id).Delete(&structures.StorageBins{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteStorageBins")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteStorageBins: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteStorageBins: %w", err)
		}

		go sseRuntime.Publish("logistics.storage_bins", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *StorageBinsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructStorageBins(pathParams)
	if err != nil {
...
func buildWhereMapFromStructStorageBins(value interface{}) (map[string]interface{}, error) {
...
func (s *StorageBinsService) DeleteStorageBinsMultiple(identities []structures.StorageBinsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructStorageBins(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.StorageBins{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteStorageBinsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteStorageBinsMultiple", "not_found_error")
		}
