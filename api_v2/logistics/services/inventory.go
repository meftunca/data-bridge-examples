// InventoryService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for Inventory resources with:
...
// IInventoryService defines the business logic interface for Inventory operations.
//
// This interface abstracts all business operations for Inventory resources, providing:
...
type IInventoryService interface {
	UpdateInventory(id types.URID, data structures.InventoryEdit) error
	UpdateInventoryMultiple(data []structures.InventoryBatchUpdate) error
	CreateInventory(data structures.InventoryForm) (structures.InventoryForm, error)
	CreateInventoryMultiple(data []structures.InventoryForm) ([]structures.InventoryForm, error)
	DeleteInventory(id types.URID) error
	DeleteInventoryMultiple(identities []structures.InventoryIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// InventoryService implements IInventoryService with enterprise-grade business logic.
//
...
//	})
func (s *InventoryService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *InventoryService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *InventoryService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyInventoryCreateDefaults(data *structures.InventoryForm) {
...
func (s *InventoryService) UpdateInventory(id types.URID, data structures.InventoryEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Inventory{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateInventory")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateInventory: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateInventory: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("logistics.inventory", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *InventoryService) UpdateInventoryMultiple(data []structures.InventoryBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateInventoryMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateInventoryMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateInventoryMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateInventoryMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.Inventory{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateInventoryMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateInventoryMultiple", "not_found_error")
		}
...
func (s *InventoryService) CreateInventory(data structures.InventoryForm) (structures.InventoryForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyInventoryCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateInventory")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateInventory: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("logistics.inventory", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *InventoryService) CreateInventoryMultiple(data []structures.InventoryForm) ([]structures.InventoryForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateInventoryMultiple", "validation_error")
	}

	for i := range data {
		applyInventoryCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateInventoryMultiple")
		}
		return nil
	})
...
func (s *InventoryService) DeleteInventory(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Inventory{}).Where("id = ?", id).Delete(&structures.Inventory{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteInventory")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteInventory: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteInventory: %w", err)
		}

		go sseRuntime.Publish("logistics.inventory", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *InventoryService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructInventory(pathParams)
	if err != nil {
...
func buildWhereMapFromStructInventory(value interface{}) (map[string]interface{}, error) {
...
func (s *InventoryService) DeleteInventoryMultiple(identities []structures.InventoryIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructInventory(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.Inventory{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteInventoryMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteInventoryMultiple", "not_found_error")
		}
