// ShipmentTrackingService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for ShipmentTracking resources with:
...
// IShipmentTrackingService defines the business logic interface for ShipmentTracking operations.
//
// This interface abstracts all business operations for ShipmentTracking resources, providing:
...
type IShipmentTrackingService interface {
	UpdateShipmentTracking(id types.URID, data structures.ShipmentTrackingEdit) error
	UpdateShipmentTrackingMultiple(data []structures.ShipmentTrackingBatchUpdate) error
	CreateShipmentTracking(data structures.ShipmentTrackingForm) (structures.ShipmentTrackingForm, error)
	CreateShipmentTrackingMultiple(data []structures.ShipmentTrackingForm) ([]structures.ShipmentTrackingForm, error)
	DeleteShipmentTracking(id types.URID) error
	DeleteShipmentTrackingMultiple(identities []structures.ShipmentTrackingIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// ShipmentTrackingService implements IShipmentTrackingService with enterprise-grade business logic.
//
...
//	})
func (s *ShipmentTrackingService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *ShipmentTrackingService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *ShipmentTrackingService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyShipmentTrackingCreateDefaults(data *structures.ShipmentTrackingForm) {
	if !data.EventTime.Valid {
		data.EventTime = types.NullTime{Time: time.Now(), Valid: true}
	}
}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *ShipmentTrackingService) UpdateShipmentTracking(id types.URID, data structures.ShipmentTrackingEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.ShipmentTracking{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateShipmentTracking")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateShipmentTracking: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "ShipmentTracking",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateShipmentTracking: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("logistics.shipment_tracking", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *ShipmentTrackingService) UpdateShipmentTrackingMultiple(data []structures.ShipmentTrackingBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateShipmentTrackingMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateShipmentTrackingMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateShipmentTrackingMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateShipmentTrackingMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.ShipmentTracking{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateShipmentTrackingMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateShipmentTrackingMultiple", "not_found_error")
		}
...
func (s *ShipmentTrackingService) CreateShipmentTracking(data structures.ShipmentTrackingForm) (structures.ShipmentTrackingForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyShipmentTrackingCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateShipmentTracking")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateShipmentTracking: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("logistics.shipment_tracking", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *ShipmentTrackingService) CreateShipmentTrackingMultiple(data []structures.ShipmentTrackingForm) ([]structures.ShipmentTrackingForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateShipmentTrackingMultiple", "validation_error")
	}

	for i := range data {
		applyShipmentTrackingCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateShipmentTrackingMultiple")
		}
		return nil
	})
...
func (s *ShipmentTrackingService) DeleteShipmentTracking(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.ShipmentTracking{}).Where("id = ?", id).Delete(&structures.ShipmentTracking{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteShipmentTracking")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteShipmentTracking: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteShipmentTracking: %w", err)
		}

		go sseRuntime.Publish("logistics.shipment_tracking", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *ShipmentTrackingService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructShipmentTracking(pathParams)
	if err != nil {
...
func buildWhereMapFromStructShipmentTracking(value interface{}) (map[string]interface{}, error) {
...
func (s *ShipmentTrackingService) DeleteShipmentTrackingMultiple(identities []structures.ShipmentTrackingIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructShipmentTracking(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.ShipmentTracking{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteShipmentTrackingMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteShipmentTrackingMultiple", "not_found_error")
		}
