// PurchaseOrdersService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for PurchaseOrders resources with:
...
// IPurchaseOrdersService defines the business logic interface for PurchaseOrders operations.
//
// This interface abstracts all business operations for PurchaseOrders resources, providing:
...
type IPurchaseOrdersService interface {
	UpdatePurchaseOrders(id types.URID, data structures.PurchaseOrdersEdit) error
	UpdatePurchaseOrdersMultiple(data []structures.PurchaseOrdersBatchUpdate) error
	CreatePurchaseOrders(data structures.PurchaseOrdersForm) (structures.PurchaseOrdersForm, error)
	CreatePurchaseOrdersMultiple(data []structures.PurchaseOrdersForm) ([]structures.PurchaseOrdersForm, error)
	DeletePurchaseOrders(id types.URID) error
	DeletePurchaseOrdersMultiple(identities []structures.PurchaseOrdersIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// PurchaseOrdersService implements IPurchaseOrdersService with enterprise-grade business logic.
//
...
//	})
func (s *PurchaseOrdersService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *PurchaseOrdersService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *PurchaseOrdersService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyPurchaseOrdersCreateDefaults(data *structures.PurchaseOrdersForm) {
	if data.Status == "" {
		data.Status = "draft"
	}
}
...
func (s *PurchaseOrdersService) UpdatePurchaseOrders(id types.URID, data structures.PurchaseOrdersEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.PurchaseOrders{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdatePurchaseOrders")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdatePurchaseOrders: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "PurchaseOrders",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdatePurchaseOrders: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("logistics.purchase_orders", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *PurchaseOrdersService) UpdatePurchaseOrdersMultiple(data []structures.PurchaseOrdersBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdatePurchaseOrdersMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdatePurchaseOrdersMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdatePurchaseOrdersMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdatePurchaseOrdersMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.PurchaseOrders{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdatePurchaseOrdersMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdatePurchaseOrdersMultiple", "not_found_error")
		}
...
func (s *PurchaseOrdersService) CreatePurchaseOrders(data structures.PurchaseOrdersForm) (structures.PurchaseOrdersForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyPurchaseOrdersCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreatePurchaseOrders")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreatePurchaseOrders: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("logistics.purchase_orders", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *PurchaseOrdersService) CreatePurchaseOrdersMultiple(data []structures.PurchaseOrdersForm) ([]structures.PurchaseOrdersForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreatePurchaseOrdersMultiple", "validation_error")
	}

	for i := range data {
		applyPurchaseOrdersCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreatePurchaseOrdersMultiple")
		}
		return nil
	})
...
func (s *PurchaseOrdersService) DeletePurchaseOrders(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.PurchaseOrders{}).Where("id = ?", id).Delete(&structures.PurchaseOrders{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeletePurchaseOrders")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeletePurchaseOrders: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeletePurchaseOrders: %w", err)
		}

		go sseRuntime.Publish("logistics.purchase_orders", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *PurchaseOrdersService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructPurchaseOrders(pathParams)
	if err != nil {
...
func buildWhereMapFromStructPurchaseOrders(value interface{}) (map[string]interface{}, error) {
...
func (s *PurchaseOrdersService) DeletePurchaseOrdersMultiple(identities []structures.PurchaseOrdersIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructPurchaseOrders(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.PurchaseOrders{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeletePurchaseOrdersMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeletePurchaseOrdersMultiple", "not_found_error")
		}
