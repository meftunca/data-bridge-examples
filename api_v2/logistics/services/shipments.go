// ShipmentsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for Shipments resources with:
...
// IShipmentsService defines the business logic interface for Shipments operations.
//
// This interface abstracts all business operations for Shipments resources, providing:
...
type IShipmentsService interface {
	UpdateShipments(id types.URID, data structures.ShipmentsEdit) error
	UpdateShipmentsMultiple(data []structures.ShipmentsBatchUpdate) error
	CreateShipments(data structures.ShipmentsForm) (structures.ShipmentsForm, error)
	CreateShipmentsMultiple(data []structures.ShipmentsForm) ([]structures.ShipmentsForm, error)
	DeleteShipments(id types.URID) error
	DeleteShipmentsMultiple(identities []structures.ShipmentsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// ShipmentsService implements IShipmentsService with enterprise-grade business logic.
//
...
//	})
func (s *ShipmentsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *ShipmentsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *ShipmentsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyShipmentsCreateDefaults(data *structures.ShipmentsForm) {

}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *ShipmentsService) UpdateShipments(id types.URID, data structures.ShipmentsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Shipments{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateShipments")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateShipments: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateShipments: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("logistics.shipments", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *ShipmentsService) UpdateShipmentsMultiple(data []structures.ShipmentsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateShipmentsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateShipmentsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateShipmentsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateShipmentsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.Shipments{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateShipmentsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateShipmentsMultiple", "not_found_error")
		}
...
func (s *ShipmentsService) CreateShipments(data structures.ShipmentsForm) (structures.ShipmentsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyShipmentsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateShipments")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateShipments: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("logistics.shipments", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *ShipmentsService) CreateShipmentsMultiple(data []structures.ShipmentsForm) ([]structures.ShipmentsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateShipmentsMultiple", "validation_error")
	}

	for i := range data {
		applyShipmentsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateShipmentsMultiple")
		}
		return nil
	})
...
func (s *ShipmentsService) DeleteShipments(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Shipments{}).Where("id = ?", id).Delete(&structures.Shipments{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteShipments")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteShipments: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteShipments: %w", err)
		}

		go sseRuntime.Publish("logistics.shipments", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *ShipmentsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructShipments(pathParams)
	if err != nil {
...
func buildWhereMapFromStructShipments(value interface{}) (map[string]interface{}, error) {
...
func (s *ShipmentsService) DeleteShipmentsMultiple(identities []structures.ShipmentsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructShipments(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.Shipments{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteShipmentsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteShipmentsMultiple", "not_found_error")
		}
