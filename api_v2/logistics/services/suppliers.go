// SuppliersService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for Suppliers resources with:
...
// ISuppliersService defines the business logic interface for Suppliers operations.
//
// This interface abstracts all business operations for Suppliers resources, providing:
...
type ISuppliersService interface {
	UpdateSuppliers(id types.URID, data structures.SuppliersEdit) error
	UpdateSuppliersMultiple(data []structures.SuppliersBatchUpdate) error
	CreateSuppliers(data structures.SuppliersForm) (structures.SuppliersForm, error)
	CreateSuppliersMultiple(data []structures.SuppliersForm) ([]structures.SuppliersForm, error)
	DeleteSuppliers(id types.URID) error
	DeleteSuppliersMultiple(identities []structures.SuppliersIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// SuppliersService implements ISuppliersService with enterprise-grade business logic.
//
...
//	})
func (s *SuppliersService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *SuppliersService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *SuppliersService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applySuppliersCreateDefaults(data *structures.SuppliersForm) {

}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *SuppliersService) UpdateSuppliers(id types.URID, data structures.SuppliersEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Suppliers{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateSuppliers")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateSuppliers: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateSuppliers: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("logistics.suppliers", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *SuppliersService) UpdateSuppliersMultiple(data []structures.SuppliersBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateSuppliersMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateSuppliersMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateSuppliersMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateSuppliersMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.Suppliers{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateSuppliersMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateSuppliersMultiple", "not_found_error")
		}
...
func (s *SuppliersService) CreateSuppliers(data structures.SuppliersForm) (structures.SuppliersForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applySuppliersCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateSuppliers")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateSuppliers: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("logistics.suppliers", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *SuppliersService) CreateSuppliersMultiple(data []structures.SuppliersForm) ([]structures.SuppliersForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateSuppliersMultiple", "validation_error")
	}

	for i := range data {
		applySuppliersCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateSuppliersMultiple")
		}
		return nil
	})
...
func (s *SuppliersService) DeleteSuppliers(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Suppliers{}).Where("id = ?", id).Delete(&structures.Suppliers{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteSuppliers")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteSuppliers: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteSuppliers: %w", err)
		}

		go sseRuntime.Publish("logistics.suppliers", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *SuppliersService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructSuppliers(pathParams)
	if err != nil {
...
func buildWhereMapFromStructSuppliers(value interface{}) (map[string]interface{}, error) {
...
func (s *SuppliersService) DeleteSuppliersMultiple(identities []structures.SuppliersIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructSuppliers(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.Suppliers{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteSuppliersMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteSuppliersMultiple", "not_found_error")
		}
