// TeamsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for Teams resources with:
...
// ITeamsService defines the business logic interface for Teams operations.
//
// This interface abstracts all business operations for Teams resources, providing:
...
type ITeamsService interface {
	UpdateTeams(id types.URID, data structures.TeamsEdit) error
	UpdateTeamsMultiple(data []structures.TeamsBatchUpdate) error
	CreateTeams(data structures.TeamsForm) (structures.TeamsForm, error)
	CreateTeamsMultiple(data []structures.TeamsForm) ([]structures.TeamsForm, error)
	DeleteTeams(id types.URID) error
	DeleteTeamsMultiple(identities []structures.TeamsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// TeamsService implements ITeamsService with enterprise-grade business logic.
//
...
//	})
func (s *TeamsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *TeamsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *TeamsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyTeamsCreateDefaults(data *structures.TeamsForm) {

}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *TeamsService) UpdateTeams(id types.URID, data structures.TeamsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Teams{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateTeams")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateTeams: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateTeams: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("iam.teams", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *TeamsService) UpdateTeamsMultiple(data []structures.TeamsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateTeamsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateTeamsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateTeamsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateTeamsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.Teams{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateTeamsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateTeamsMultiple", "not_found_error")
		}
...
func (s *TeamsService) CreateTeams(data structures.TeamsForm) (structures.TeamsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyTeamsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateTeams")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateTeams: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("iam.teams", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *TeamsService) CreateTeamsMultiple(data []structures.TeamsForm) ([]structures.TeamsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateTeamsMultiple", "validation_error")
	}

	for i := range data {
		applyTeamsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateTeamsMultiple")
		}
		return nil
	})
...
func (s *TeamsService) DeleteTeams(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Teams{}).Where("id = ?", id).Delete(&structures.Teams{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteTeams")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteTeams: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteTeams: %w", err)
		}

		go sseRuntime.Publish("iam.teams", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *TeamsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructTeams(pathParams)
	if err != nil {
...
func buildWhereMapFromStructTeams(value interface{}) (map[string]interface{}, error) {
...
func (s *TeamsService) DeleteTeamsMultiple(identities []structures.TeamsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructTeams(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.Teams{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteTeamsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteTeamsMultiple", "not_found_error")
		}
