// OrganizationsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for Organizations resources with:
...
// IOrganizationsService defines the business logic interface for Organizations operations.
//
// This interface abstracts all business operations for Organizations resources, providing:
...
type IOrganizationsService interface {
	UpdateOrganizations(id types.URID, data structures.OrganizationsEdit) error
	UpdateOrganizationsMultiple(data []structures.OrganizationsBatchUpdate) error
	CreateOrganizations(data structures.OrganizationsForm) (structures.OrganizationsForm, error)
	CreateOrganizationsMultiple(data []structures.OrganizationsForm) ([]structures.OrganizationsForm, error)
	DeleteOrganizations(id types.URID) error
	DeleteOrganizationsMultiple(identities []structures.OrganizationsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// OrganizationsService implements IOrganizationsService with enterprise-grade business logic.
//
...
//	})
func (s *OrganizationsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *OrganizationsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *OrganizationsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyOrganizationsCreateDefaults(data *structures.OrganizationsForm) {

}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *OrganizationsService) UpdateOrganizations(id types.URID, data structures.OrganizationsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Organizations{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateOrganizations")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateOrganizations: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateOrganizations: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("iam.organizations", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *OrganizationsService) UpdateOrganizationsMultiple(data []structures.OrganizationsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateOrganizationsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateOrganizationsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateOrganizationsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateOrganizationsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.Organizations{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateOrganizationsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateOrganizationsMultiple", "not_found_error")
		}
...
func (s *OrganizationsService) CreateOrganizations(data structures.OrganizationsForm) (structures.OrganizationsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyOrganizationsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateOrganizations")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateOrganizations: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("iam.organizations", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *OrganizationsService) CreateOrganizationsMultiple(data []structures.OrganizationsForm) ([]structures.OrganizationsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateOrganizationsMultiple", "validation_error")
	}

	for i := range data {
		applyOrganizationsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateOrganizationsMultiple")
		}
		return nil
	})
...
func (s *OrganizationsService) DeleteOrganizations(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Organizations{}).Where("id = ?", id).Delete(&structures.Organizations{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteOrganizations")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteOrganizations: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteOrganizations: %w", err)
		}

		go sseRuntime.Publish("iam.organizations", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *OrganizationsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructOrganizations(pathParams)
	if err != nil {
...
func buildWhereMapFromStructOrganizations(value interface{}) (map[string]interface{}, error) {
...
func (s *OrganizationsService) DeleteOrganizationsMultiple(identities []structures.OrganizationsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructOrganizations(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.Organizations{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteOrganizationsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteOrganizationsMultiple", "not_found_error")
		}
