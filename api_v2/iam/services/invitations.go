// InvitationsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for Invitations resources with:
...
// IInvitationsService defines the business logic interface for Invitations operations.
//
// This interface abstracts all business operations for Invitations resources, providing:
...
type IInvitationsService interface {
	UpdateInvitations(id types.URID, data structures.InvitationsEdit) error
	UpdateInvitationsMultiple(data []structures.InvitationsBatchUpdate) error
	CreateInvitations(data structures.InvitationsForm) (structures.InvitationsForm, error)
	CreateInvitationsMultiple(data []structures.InvitationsForm) ([]structures.InvitationsForm, error)
	DeleteInvitations(id types.URID) error
	DeleteInvitationsMultiple(identities []structures.InvitationsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// InvitationsService implements IInvitationsService with enterprise-grade business logic.
//
...
//	})
func (s *InvitationsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *InvitationsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *InvitationsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyInvitationsCreateDefaults(data *structures.InvitationsForm) {

}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *InvitationsService) UpdateInvitations(id types.URID, data structures.InvitationsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Invitations{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateInvitations")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateInvitations: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "Invitations",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateInvitations: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("iam.invitations", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *InvitationsService) UpdateInvitationsMultiple(data []structures.InvitationsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateInvitationsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateInvitationsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateInvitationsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateInvitationsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.Invitations{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateInvitationsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateInvitationsMultiple", "not_found_error")
		}
...
func (s *InvitationsService) CreateInvitations(data structures.InvitationsForm) (structures.InvitationsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyInvitationsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateInvitations")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateInvitations: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("iam.invitations", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *InvitationsService) CreateInvitationsMultiple(data []structures.InvitationsForm) ([]structures.InvitationsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateInvitationsMultiple", "validation_error")
	}

	for i := range data {
		applyInvitationsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateInvitationsMultiple")
		}
		return nil
	})
...
func (s *InvitationsService) DeleteInvitations(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Invitations{}).Where("id = ?", id).Delete(&structures.Invitations{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteInvitations")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteInvitations: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteInvitations: %w", err)
		}

		go sseRuntime.Publish("iam.invitations", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *InvitationsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructInvitations(pathParams)
	if err != nil {
...
func buildWhereMapFromStructInvitations(value interface{}) (map[string]interface{}, error) {
...
func (s *InvitationsService) DeleteInvitationsMultiple(identities []structures.InvitationsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructInvitations(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.Invitations{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteInvitationsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteInvitationsMultiple", "not_found_error")
		}
