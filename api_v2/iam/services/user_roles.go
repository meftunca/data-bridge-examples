// UserRolesService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for UserRoles resources with:
...
// IUserRolesService defines the business logic interface for UserRoles operations.
//
// This interface abstracts all business operations for UserRoles resources, providing:
...
type IUserRolesService interface {
	UpdateUserRoles(id types.URID, data structures.UserRolesEdit) error
	UpdateUserRolesMultiple(data []structures.UserRolesBatchUpdate) error
	CreateUserRoles(data structures.UserRolesForm) (structures.UserRolesForm, error)
	CreateUserRolesMultiple(data []structures.UserRolesForm) ([]structures.UserRolesForm, error)
	DeleteUserRoles(id types.URID) error
	DeleteUserRolesMultiple(identities []structures.UserRolesIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// UserRolesService implements IUserRolesService with enterprise-grade business logic.
//
...
//	})
func (s *UserRolesService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *UserRolesService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *UserRolesService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyUserRolesCreateDefaults(data *structures.UserRolesForm) {
	if !data.GrantedAt.Valid {
		data.GrantedAt = types.NullTime{Time: time.Now(), Valid: true}
	}
}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *UserRolesService) UpdateUserRoles(id types.URID, data structures.UserRolesEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.UserRoles{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateUserRoles")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateUserRoles: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateUserRoles: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("iam.user_roles", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *UserRolesService) UpdateUserRolesMultiple(data []structures.UserRolesBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateUserRolesMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateUserRolesMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateUserRolesMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateUserRolesMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.UserRoles{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateUserRolesMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateUserRolesMultiple", "not_found_error")
		}
...
func (s *UserRolesService) CreateUserRoles(data structures.UserRolesForm) (structures.UserRolesForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyUserRolesCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateUserRoles")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateUserRoles: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("iam.user_roles", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *UserRolesService) CreateUserRolesMultiple(data []structures.UserRolesForm) ([]structures.UserRolesForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateUserRolesMultiple", "validation_error")
	}

	for i := range data {
		applyUserRolesCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateUserRolesMultiple")
		}
		return nil
	})
...
func (s *UserRolesService) DeleteUserRoles(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.UserRoles{}).Where("id = ?", id).Delete(&structures.UserRoles{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteUserRoles")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteUserRoles: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteUserRoles: %w", err)
		}

		go sseRuntime.Publish("iam.user_roles", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *UserRolesService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructUserRoles(pathParams)
	if err != nil {
...
func buildWhereMapFromStructUserRoles(value interface{}) (map[string]interface{}, error) {
...
func (s *UserRolesService) DeleteUserRolesMultiple(identities []structures.UserRolesIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructUserRoles(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.UserRoles{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteUserRolesMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteUserRolesMultiple", "not_found_error")
		}
