// ApiKeysService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for ApiKeys resources with:
...
// IApiKeysService defines the business logic interface for ApiKeys operations.
//
// This interface abstracts all business operations for ApiKeys resources, providing:
...
type IApiKeysService interface {
	UpdateApiKeys(id types.URID, data structures.ApiKeysEdit) error
	UpdateApiKeysMultiple(data []structures.ApiKeysBatchUpdate) error
	CreateApiKeys(data structures.ApiKeysForm) (structures.ApiKeysForm, error)
	CreateApiKeysMultiple(data []structures.ApiKeysForm) ([]structures.ApiKeysForm, error)
	DeleteApiKeys(id types.URID) error
	DeleteApiKeysMultiple(identities []structures.ApiKeysIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// ApiKeysService implements IApiKeysService with enterprise-grade business logic.
//
...
//	})
func (s *ApiKeysService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *ApiKeysService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *ApiKeysService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyApiKeysCreateDefaults(data *structures.ApiKeysForm) {

}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *ApiKeysService) UpdateApiKeys(id types.URID, data structures.ApiKeysEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.ApiKeys{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateApiKeys")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateApiKeys: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "ApiKeys",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateApiKeys: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("iam.api_keys", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *ApiKeysService) UpdateApiKeysMultiple(data []structures.ApiKeysBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateApiKeysMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateApiKeysMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateApiKeysMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateApiKeysMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.ApiKeys{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateApiKeysMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateApiKeysMultiple", "not_found_error")
		}
...
func (s *ApiKeysService) CreateApiKeys(data structures.ApiKeysForm) (structures.ApiKeysForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyApiKeysCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateApiKeys")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateApiKeys: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("iam.api_keys", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *ApiKeysService) CreateApiKeysMultiple(data []structures.ApiKeysForm) ([]structures.ApiKeysForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateApiKeysMultiple", "validation_error")
	}

	for i := range data {
		applyApiKeysCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateApiKeysMultiple")
		}
		return nil
	})
...
func (s *ApiKeysService) DeleteApiKeys(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.ApiKeys{}).Where("id = ?", id).Delete(&structures.ApiKeys{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteApiKeys")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteApiKeys: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteApiKeys: %w", err)
		}

		go sseRuntime.Publish("iam.api_keys", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *ApiKeysService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructApiKeys(pathParams)
	if err != nil {
...
func buildWhereMapFromStructApiKeys(value interface{}) (map[string]interface{}, error) {
...
func (s *ApiKeysService) DeleteApiKeysMultiple(identities []structures.ApiKeysIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructApiKeys(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.ApiKeys{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteApiKeysMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteApiKeysMultiple", "not_found_error")
		}
