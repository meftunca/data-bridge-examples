// SessionsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for Sessions resources with:
...
// ISessionsService defines the business logic interface for Sessions operations.
//
// This interface abstracts all business operations for Sessions resources, providing:
...
type ISessionsService interface {
	UpdateSessions(id types.URID, data structures.SessionsEdit) error
	UpdateSessionsMultiple(data []structures.SessionsBatchUpdate) error
	CreateSessions(data structures.SessionsForm) (structures.SessionsForm, error)
	CreateSessionsMultiple(data []structures.SessionsForm) ([]structures.SessionsForm, error)
	DeleteSessions(id types.URID) error
	DeleteSessionsMultiple(identities []structures.SessionsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// SessionsService implements ISessionsService with enterprise-grade business logic.
//
...
//	})
func (s *SessionsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *SessionsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *SessionsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applySessionsCreateDefaults(data *structures.SessionsForm) {

}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *SessionsService) UpdateSessions(id types.URID, data structures.SessionsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Sessions{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateSessions")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateSessions: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "Sessions",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateSessions: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("iam.sessions", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *SessionsService) UpdateSessionsMultiple(data []structures.SessionsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateSessionsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateSessionsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateSessionsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateSessionsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.Sessions{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateSessionsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateSessionsMultiple", "not_found_error")
		}
...
func (s *SessionsService) CreateSessions(data structures.SessionsForm) (structures.SessionsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applySessionsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateSessions")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateSessions: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("iam.sessions", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *SessionsService) CreateSessionsMultiple(data []structures.SessionsForm) ([]structures.SessionsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateSessionsMultiple", "validation_error")
	}

	for i := range data {
		applySessionsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateSessionsMultiple")
		}
		return nil
	})
...
func (s *SessionsService) DeleteSessions(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Sessions{}).Where("id = ?", id).Delete(&structures.Sessions{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteSessions")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteSessions: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteSessions: %w", err)
		}

		go sseRuntime.Publish("iam.sessions", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *SessionsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructSessions(pathParams)
	if err != nil {
...
func buildWhereMapFromStructSessions(value interface{}) (map[string]interface{}, error) {
...
func (s *SessionsService) DeleteSessionsMultiple(identities []structures.SessionsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructSessions(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.Sessions{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteSessionsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteSessionsMultiple", "not_found_error")
		}
