// TeamMembersService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for TeamMembers resources with:
...
// ITeamMembersService defines the business logic interface for TeamMembers operations.
//
// This interface abstracts all business operations for TeamMembers resources, providing:
...
type ITeamMembersService interface {
	UpdateTeamMembers(id types.URID, data structures.TeamMembersEdit) error
	UpdateTeamMembersMultiple(data []structures.TeamMembersBatchUpdate) error
	CreateTeamMembers(data structures.TeamMembersForm) (structures.TeamMembersForm, error)
	CreateTeamMembersMultiple(data []structures.TeamMembersForm) ([]structures.TeamMembersForm, error)
	DeleteTeamMembers(id types.URID) error
	DeleteTeamMembersMultiple(identities []structures.TeamMembersIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// TeamMembersService implements ITeamMembersService with enterprise-grade business logic.
//
...
//	})
func (s *TeamMembersService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *TeamMembersService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *TeamMembersService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyTeamMembersCreateDefaults(data *structures.TeamMembersForm) {
	if data.Role == "" {
		data.Role = "member"
	}
	if !data.JoinedAt.Valid {
		data.JoinedAt = types.NullTime{Time: time.Now(), Valid: true}
	}
}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *TeamMembersService) UpdateTeamMembers(id types.URID, data structures.TeamMembersEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.TeamMembers{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateTeamMembers")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateTeamMembers: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "TeamMembers",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateTeamMembers: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("iam.team_members", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *TeamMembersService) UpdateTeamMembersMultiple(data []structures.TeamMembersBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateTeamMembersMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateTeamMembersMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateTeamMembersMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateTeamMembersMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.TeamMembers{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateTeamMembersMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateTeamMembersMultiple", "not_found_error")
		}
...
func (s *TeamMembersService) CreateTeamMembers(data structures.TeamMembersForm) (structures.TeamMembersForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyTeamMembersCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateTeamMembers")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateTeamMembers: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("iam.team_members", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *TeamMembersService) CreateTeamMembersMultiple(data []structures.TeamMembersForm) ([]structures.TeamMembersForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateTeamMembersMultiple", "validation_error")
	}

	for i := range data {
		applyTeamMembersCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateTeamMembersMultiple")
		}
		return nil
	})
...
func (s *TeamMembersService) DeleteTeamMembers(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.TeamMembers{}).Where("id = ?", id).Delete(&structures.TeamMembers{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteTeamMembers")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteTeamMembers: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteTeamMembers: %w", err)
		}

		go sseRuntime.Publish("iam.team_members", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *TeamMembersService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructTeamMembers(pathParams)
	if err != nil {
...
func buildWhereMapFromStructTeamMembers(value interface{}) (map[string]interface{}, error) {
...
func (s *TeamMembersService) DeleteTeamMembersMultiple(identities []structures.TeamMembersIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructTeamMembers(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.TeamMembers{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteTeamMembersMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteTeamMembersMultiple", "not_found_error")
		}
