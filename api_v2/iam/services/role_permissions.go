// RolePermissionsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for RolePermissions resources with:
...
// IRolePermissionsService defines the business logic interface for RolePermissions operations.
//
// This interface abstracts all business operations for RolePermissions resources, providing:
...
type IRolePermissionsService interface {
	UpdateRolePermissions(id types.URID, data structures.RolePermissionsEdit) error
	UpdateRolePermissionsMultiple(data []structures.RolePermissionsBatchUpdate) error
	CreateRolePermissions(data structures.RolePermissionsForm) (structures.RolePermissionsForm, error)
	CreateRolePermissionsMultiple(data []structures.RolePermissionsForm) ([]structures.RolePermissionsForm, error)
	DeleteRolePermissions(id types.URID) error
	DeleteRolePermissionsMultiple(identities []structures.RolePermissionsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// RolePermissionsService implements IRolePermissionsService with enterprise-grade business logic.
//
...
//	})
func (s *RolePermissionsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *RolePermissionsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *RolePermissionsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyRolePermissionsCreateDefaults(data *structures.RolePermissionsForm) {

}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *RolePermissionsService) UpdateRolePermissions(id types.URID, data structures.RolePermissionsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.RolePermissions{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateRolePermissions")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateRolePermissions: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "RolePermissions",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateRolePermissions: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("iam.role_permissions", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *RolePermissionsService) UpdateRolePermissionsMultiple(data []structures.RolePermissionsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateRolePermissionsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateRolePermissionsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateRolePermissionsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateRolePermissionsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.RolePermissions{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateRolePermissionsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateRolePermissionsMultiple", "not_found_error")
		}
...
func (s *RolePermissionsService) CreateRolePermissions(data structures.RolePermissionsForm) (structures.RolePermissionsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyRolePermissionsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateRolePermissions")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateRolePermissions: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("iam.role_permissions", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *RolePermissionsService) CreateRolePermissionsMultiple(data []structures.RolePermissionsForm) ([]structures.RolePermissionsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateRolePermissionsMultiple", "validation_error")
	}

	for i := range data {
		applyRolePermissionsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateRolePermissionsMultiple")
		}
		return nil
	})
...
func (s *RolePermissionsService) DeleteRolePermissions(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.RolePermissions{}).Where("id = ?", id).Delete(&structures.RolePermissions{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteRolePermissions")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteRolePermissions: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteRolePermissions: %w", err)
		}

		go sseRuntime.Publish("iam.role_permissions", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *RolePermissionsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructRolePermissions(pathParams)
	if err != nil {
...
func buildWhereMapFromStructRolePermissions(value interface{}) (map[string]interface{}, error) {
...
func (s *RolePermissionsService) DeleteRolePermissionsMultiple(identities []structures.RolePermissionsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructRolePermissions(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.RolePermissions{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteRolePermissionsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteRolePermissionsMultiple", "not_found_error")
		}
