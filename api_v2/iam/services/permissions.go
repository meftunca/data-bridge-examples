// PermissionsService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for Permissions resources with:
...
// IPermissionsService defines the business logic interface for Permissions operations.
//
// This interface abstracts all business operations for Permissions resources, providing:
...
type IPermissionsService interface {
	UpdatePermissions(id types.URID, data structures.PermissionsEdit) error
	UpdatePermissionsMultiple(data []structures.PermissionsBatchUpdate) error
	CreatePermissions(data structures.PermissionsForm) (structures.PermissionsForm, error)
	CreatePermissionsMultiple(data []structures.PermissionsForm) ([]structures.PermissionsForm, error)
	DeletePermissions(id types.URID) error
	DeletePermissionsMultiple(identities []structures.PermissionsIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// PermissionsService implements IPermissionsService with enterprise-grade business logic.
//
...
//	})
func (s *PermissionsService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *PermissionsService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *PermissionsService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyPermissionsCreateDefaults(data *structures.PermissionsForm) {
	if data.Resource == "" {
		data.Resource = "*"
	}
	if data.Action == "" {
...
func (s *PermissionsService) UpdatePermissions(id types.URID, data structures.PermissionsEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Permissions{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdatePermissions")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdatePermissions: %w", gorm.ErrRecordNotFound)
		}

		// Transactional outbox: publish update event atomically in same transaction
		payloadBytes, _ := json.Marshal(data)
		outboxEvt := OutboxEvent{
			AggregateType: "Permissions",
			EventType:     "updated",
			Payload:       string(payloadBytes),
		}
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdatePermissions: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("iam.permissions", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *PermissionsService) UpdatePermissionsMultiple(data []structures.PermissionsBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdatePermissionsMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdatePermissionsMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdatePermissionsMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdatePermissionsMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.Permissions{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdatePermissionsMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdatePermissionsMultiple", "not_found_error")
		}
...
func (s *PermissionsService) CreatePermissions(data structures.PermissionsForm) (structures.PermissionsForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyPermissionsCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreatePermissions")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreatePermissions: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("iam.permissions", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *PermissionsService) CreatePermissionsMultiple(data []structures.PermissionsForm) ([]structures.PermissionsForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreatePermissionsMultiple", "validation_error")
	}

	for i := range data {
		applyPermissionsCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreatePermissionsMultiple")
		}
		return nil
	})
...
func (s *PermissionsService) DeletePermissions(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Permissions{}).Where("id = ?", id).Delete(&structures.Permissions{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeletePermissions")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeletePermissions: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeletePermissions: %w", err)
		}

		go sseRuntime.Publish("iam.permissions", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *PermissionsService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructPermissions(pathParams)
	if err != nil {
...
func buildWhereMapFromStructPermissions(value interface{}) (map[string]interface{}, error) {
...
func (s *PermissionsService) DeletePermissionsMultiple(identities []structures.PermissionsIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructPermissions(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.Permissions{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeletePermissionsMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeletePermissionsMultiple", "not_found_error")
		}
