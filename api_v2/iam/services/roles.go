// RolesService - Enterprise-Grade Business Logic Service
//
// This service implements comprehensive business logic for Roles resources with:
...
// IRolesService defines the business logic interface for Roles operations.
//
// This interface abstracts all business operations for Roles resources, providing:
...
type IRolesService interface {
	UpdateRoles(id types.URID, data structures.RolesEdit) error
	UpdateRolesMultiple(data []structures.RolesBatchUpdate) error
	CreateRoles(data structures.RolesForm) (structures.RolesForm, error)
	CreateRolesMultiple(data []structures.RolesForm) ([]structures.RolesForm, error)
	DeleteRoles(id types.URID) error
	DeleteRolesMultiple(identities []structures.RolesIdentity) error // Tüm silme işlemleri tek bir transaction içinde yapılır.
}

// RolesService implements IRolesService with enterprise-grade business logic.
//
...
//	})
func (s *RolesService) withTransaction(ctx context.Context, fn func(tx *gorm.DB) error) error {
...
func (s *RolesService) handleServiceError(err error, operation string) error {
	if err == nil {
		return nil
	}
...
func (s *RolesService) validateContext(ctx context.Context) error {
	if ctx == nil {
...
func applyRolesCreateDefaults(data *structures.RolesForm) {

}

// Update, verilen primary/unique key'e göre bir kaydı günceller.
func (s *RolesService) UpdateRoles(id types.URID, data structures.RolesEdit) error {
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Roles{}).Where("id = ?", id).Updates(data)

		if result.Error != nil {
			return s.handleServiceError(result.Error, "UpdateRoles")
		}

		// Check if any record was actually updated
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to update in UpdateRoles: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in UpdateRoles: %w", err)
		}

		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("iam.roles", sseRuntime.Event{Type: "updated", Payload: evtBytes})
...
func (s *RolesService) UpdateRolesMultiple(data []structures.RolesBatchUpdate) error {
	...
	if len(data) == 0 {
		return fmt.Errorf("%s: empty data slice in UpdateRolesMultiple", "validation_error")
	}
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, item := range data {
			...
			if err != nil {
				return fmt.Errorf("%s: invalid data format for conversion at index %d in UpdateRolesMultiple: %w", "validation_error", i, err)
			}

			var updateData map[string]interface{}
			if err := json.Unmarshal(updateDataBytes, &updateData); err != nil {
				return fmt.Errorf("%s: failed to unmarshal update data at index %d in UpdateRolesMultiple: %w", "validation_error", i, err)
			}
			...
			if err != nil {
				return fmt.Errorf("%s: failed to build where clause at index %d in UpdateRolesMultiple: %w", "validation_error", i, err)
			}

			result := tx.Model(&structures.Roles{}).Where(whereClause).Updates(updateData)
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("UpdateRolesMultiple[%d]", i))
			}
		...
		if totalAffected == 0 {
			return fmt.Errorf("%s: no records were updated in UpdateRolesMultiple", "not_found_error")
		}
...
func (s *RolesService) CreateRoles(data structures.RolesForm) (structures.RolesForm, error) {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	applyRolesCreateDefaults(&data)

	// Execute create within transaction
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		if err := tx.Create(&data).Error; err != nil {
			return s.handleServiceError(err, "CreateRoles")
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in CreateRoles: %w", err)
		}

		// SSE: fan-out to local subscribers (best-effort, post-write broadcast)
		evtBytes, _ := json.Marshal(data)
		go sseRuntime.Publish("iam.roles", sseRuntime.Event{Type: "created", Payload: evtBytes})
...
func (s *RolesService) CreateRolesMultiple(data []structures.RolesForm) ([]structures.RolesForm, error) {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	if len(data) == 0 {
		return data, fmt.Errorf("%s: empty data slice in CreateRolesMultiple", "validation_error")
	}

	for i := range data {
		applyRolesCreateDefaults(&data[i])
	}
	...
	err := s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		if err := tx.CreateInBatches(&data, batchSize).Error; err != nil {
			return s.handleServiceError(err, "CreateRolesMultiple")
		}
		return nil
	})
...
func (s *RolesService) DeleteRoles(id types.URID) error {
	// Create timeout context
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		result := tx.Model(&structures.Roles{}).Where("id = ?", id).Delete(&structures.Roles{})

		if result.Error != nil {
			return s.handleServiceError(result.Error, "DeleteRoles")
		}

		// Check if any record was actually deleted
		if result.RowsAffected == 0 {
			return fmt.Errorf("not_found_error: no record found to delete in DeleteRoles: %w", gorm.ErrRecordNotFound)
		}
		...
		if err := tx.Table("outbox_events").Create(&outboxEvt).Error; err != nil {
			return fmt.Errorf("outbox insert failed in DeleteRoles: %w", err)
		}

		go sseRuntime.Publish("iam.roles", sseRuntime.Event{Type: "deleted", Payload: []byte("{}")})
...
func (s *RolesService) buildWhereFromPathParams(pathParams interface{}) (map[string]interface{}, error) {
	if pathParams == nil {
		return nil, fmt.Errorf("%s: pathParams is nil", "validation_error")
	}

	whereMap, err := buildWhereMapFromStructRoles(pathParams)
	if err != nil {
...
func buildWhereMapFromStructRoles(value interface{}) (map[string]interface{}, error) {
...
func (s *RolesService) DeleteRolesMultiple(identities []structures.RolesIdentity) error {
	// Create timeout context for batch operation
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	...
	return s.withTransaction(ctx, func(tx *gorm.DB) error {
		...
		for i, identity := range identities {
			...
			whereConditions, err := buildWhereMapFromStructRoles(identity)
			if err != nil {
			...
			result := tx.Where(whereConditions).Delete(&structures.Roles{})
			if result.Error != nil {
				return s.handleServiceError(result.Error, fmt.Sprintf("DeleteRolesMultiple[%d]", i))
			}
		...
		if totalDeleted == 0 {
			return fmt.Errorf("%s: no records were deleted in DeleteRolesMultiple", "not_found_error")
		}
